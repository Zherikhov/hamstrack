...
3. **Counts are part of every report, and the guard's own lines are the evidence.** Quote the `[test-tree]` and `[vitest-tree]` lines verbatim, and whenever an arm stood down quote its disarm line too — a disarmed arm that goes unmentioned reads exactly like an armed one. The two arms do not disarm together: `-Dtest=` narrows surefire and leaves vitest armed, `-Dfrontend.skip=true` is the reverse. `Tests run:` with the class count, and the vitest file and test counts, are the numbers those lines are about. A green number nobody compared once hid 28 unexecuted classes. These two lines exist only on a run that **reached the `test` phase's last execution**: surefire, `npm-lint` and `npm-test` all abort Maven before the guard, so a run red in any of them has no `[test-tree]`/`[vitest-tree]` line to quote and `target/test-tree-summary.md` still holds the *previous* invocation's table — check its header line, which names the run that wrote it. “The guard did not run, because &lt;which step&gt; failed first” is then the honest report; quoting a stale summary is not.
