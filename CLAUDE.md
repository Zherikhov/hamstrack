...
- **`WorkspaceAccessService.requireProjectMember` does NOT check project membership — it checks *workspace* membership** (`workspace/service/WorkspaceAccessService.java:70`). It resolves the workspace, verifies the actor is a member of *it*, then finds the project inside that workspace. The name reads like a project-membership gate and is not one. Consequences to know before you reason about authorization: every issue mutation (create/update/status/assign/rank/comment) routes through it, so **any workspace member can edit issues in every project of the workspace**; `ProjectRole.VIEWER` is only the fallback returned when no `project_members` row exists (`ProjectService.java:214`, `IssueService.java:962`) and `isAtLeast(VIEWER)` is always true, so `requireRole(…, VIEWER)` is a gate everyone passes and nothing in the product means "read-only". Exactly two real project-level boundaries exist: `IssueService.delete` (MANAGER, `IssueService.java:712`) and `ScopeResolver.requireProjectCurator` (project MANAGER **or** workspace OWNER/ADMIN, 26 call sites). This is current *intended* behavior, not a bug to fix in passing — HD-123 is the epic that replaces it. Do not "tighten" it as a side effect of another change: that is a breaking change for every existing install. Related trap: all three role enums compare via `isAtLeast` = `ordinal() <=`, which hard-codes a **total order** over roles; anything custom or composable has no such order, so those call sites must become permission checks, not new enum values.
