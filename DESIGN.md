...
- **Top bar (cool exception to the warm palette):** diagonal gradient `135deg`, near-black navy `#0B0D16` for the first 30%, easing into navy `#263764`; dropdowns anchored to the bar use `#272E48`. The rest of the UI stays warm-neutral.
...
| 2026-07-13 | Top bar recolored from flat warm ink `#1C1B19` to a near-black-navy→navy diagonal gradient (30% near-black, end color `#263764` — tuned from the initial 70%/`#1B2745` in two rounds for a clearly visible blue) and grown 52→56px; its dropdowns follow (`#272E48`) | Explicit user request for a subtle navy identity on the global bar; kept deliberately close to black so the cool hue reads as depth, not a palette break — all other surfaces stay warm-neutral |
| 2026-07-13 | Board columns are fluid (`flex: 1 1 280px`, min 240 / max 420) instead of fixed 280px | Board should use the full viewport width on large screens; below the minimum, columns overflow into horizontal scroll as before |
| 2026-07-12 | Public surfaces (landing `/`, legal pages, cookie notice) reuse the app palette: warm-neutral surface, Cabinet Grotesk hero (52/800), 1080px max-width, teal CTAs; legal prose column 720px with mono cookie table; cookie notice is a small dark-ink card bottom-left with a single "Got it" action | Marketing surfaces stay in the industrial/utilitarian register rather than a separate breezy landing style; mono table for the cookie inventory extends the "inspectable, not magic" typography rule; informational (not consent) banner because the only cookie is strictly necessary |
