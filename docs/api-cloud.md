...
It names projects and their volumes — including projects the caller is not a member of — so it is real disclosure and requires **`workspace.edit`**: `403` naming the permission for a proven member without it, `404` (never `403`) for a non-member or an unknown workspace. And it is a grouped aggregate over every attachment row in the workspace — work proportional to what the tenant holds rather than to what the response returns — so it spends the **[reports budget](#rate-limits)** (60 requests per minute per user by default) and answers `429` + `Retry-After` past it. The asymmetry with the unbudgeted summary runs the safe way round: a caller refused here who falls back to the summary gets *less* information, never a way around the bound.
