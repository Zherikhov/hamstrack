...
**The planning reads have a pot of their own, and it is sized for a gesture rather than for a chart.** The whole view [`GET …/projects/{pId}/backlog`](#the-planning-view) and [every section of it](#refreshing-one-section) under `…/backlog/sections/…` share **one per-principal budget of 240 requests per minute**, bound to the whole `…/backlog/**` path rather than to a list of endpoints, so a planning read added later is inside it the moment it exists. It is counted **in memory per app node**, like the other per-principal budgets above, so a deployment running several instances gives one user that budget *per node*: it is a bound on abuse and never a quota to quote — do not build a client that meters itself against 240. It is deliberately not the reports pot: a card dragged across sections costs two section refreshes, so an ordinary grooming session would reach 60/min inside three minutes, and a `429` in the middle of a drag reads as a product defect where a `429` on a chart reads as a delay. Past it the answer is `429` + `Retry-After` — never a narrowed section, a smaller `sectionCap` or a `truncated` that was not earned.
...
**None of this is a setting of yours.** The numbers behind the bound belong to the instance, are the same for every workspace and every plan, and are deliberately never varied per tenant — an occupancy share that moved with a plan would be a licence check wearing a resource guard's clothes. Nothing you can send changes them, so what a refusal asks of you is on the list above: let one of your own requests finish, or wait a moment and try again. The bound is counted per app node, because the connections it protects are, so it is not a fleet-wide quota and should not be quoted as one.

**Writing costs a budget too, and the write surface had none until recently.** Every **mutating** request under `…/projects/*/issues/**` — issue create/update/delete, comment create/update/delete, attachment upload/delete and [backlog rank](#ranking-an-issue) — spends a **per-principal budget of its own** — **180 requests per minute** (in-memory per app node, like the other per-principal budgets above). It is method-conditioned, so `GET`s on the same paths cost nothing, and it covers `PATCH` and `DELETE` as well as `POST`: a budget that stopped only creation would simply move a caller to the update, and an issue update is not the cheaper request — it writes history rows, bumps the version, and fans out notifications and [SSE](#real-time-events-sse). Uploading an attachment additionally spends a per-principal **byte** budget of **250 MB per minute**, summed over the **parsed** sizes of your own files, because a request count does not bound bytes. Both answer `429` + `Retry-After`, both are spent before the workspace is resolved, and both are keyed on the caller — so neither refusal says anything about a workspace, and neither is affected by what anybody else is doing. The rank endpoint can therefore answer `429` for either reason, and `detail` says which.
...
| `POST` | `/auth/reset-password` | — | Set a new password with the emailed token; revokes all sessions **and [every other outstanding reset link](#redeeming-a-reset-link-retires-every-other-outstanding-one) for that account**. `422` when `newPassword` cannot be stored as sent — [a value this project published](#a-password-this-project-published--a-422), or one [longer than 72 UTF-8 bytes](#a-password-longer-than-72-utf-8-bytes--a-422); the token stays usable either way |
...
**The master switch takes all of them with it.** Where rate limiting is disabled instance-wide (`app.rate-limit.enabled=false`), this endpoint stops answering `429` altogether — the ceilings are not raised, they are not consulted. Nothing else about the endpoint changes.
...
**Read `quotaEnabled` first and narrow on it.** With the quota switched off there is no ceiling, so `quotaBytes`, `availableBytes` and `percentUsed` are **`null`** — absent numbers rather than a sentinel, because a `-1` is a value a client eventually renders as "-1 GB remaining" and a `0` reads as "full". Everything that describes *usage* rather than the ceiling is always present: usage is counted and reported whether or not anything is being refused, so a client can render the position even on an instance that enforces no ceiling at all. Bytes are raw integers everywhere; formatting is the client's business. **Read the ceiling from this endpoint rather than hardcoding it** — it is an instance setting, and the figure here is the one being enforced.

`percentUsed` is **not clamped at 100**: if the ceiling is ever lowered below what a workspace already holds, existing files stay readable and new uploads are refused — clamping the number would hide the one state that explains those refusals. `availableBytes` *is* clamped at 0, because "-2.1 GB remaining" is not something a reader can act on. `asOf` is the usage counter's own timestamp and is `null` for a workspace that has never held an attachment. `maxFileBytes` is the per-file ceiling (default 20 MB), carried here so one response answers both "will this file be accepted" questions. `warnAtPercent` changes no server behaviour at all — nothing is refused, narrowed or slowed at that level; it exists so the first thing a workspace hears about its ceiling is not a refusal.
...
It names projects and their volumes — including projects the caller is not a member of — so it is real disclosure and requires **`workspace.edit`**: `403` naming the permission for a proven member without it, `404` (never `403`) for a non-member or an unknown workspace. And it is a grouped aggregate over every attachment row in the workspace — work proportional to what the tenant holds rather than to what the response returns — so it spends the **[reports budget](#rate-limits)** (60 requests per minute per user by default) and is inside the same bound on how many expensive reads may run at once. Past either it answers `429` + `Retry-After`, with `errorType` naming which — an absent one is the budget (see [Rate limits](#rate-limits)). The asymmetry with the unbudgeted summary runs the safe way round: a caller refused here who falls back to the summary gets *less* information, never a way around the bound.
...
**Nobody calls this endpoint by hand, and no integration should.** Every response this instance sends carries a `Content-Security-Policy-Report-Only` header, and that header's `report-uri` names this exact path; when a page breaks the policy, the browser posts a report here by itself. That is the entire caller list, and reading the statuses below as answers to a *person* is what makes them look strange. The sink stores nothing, hands nothing back, and is not workspace-scoped — a report is emitted by a browser rather than by a session, and carries no tenant identity that could be trusted if it did. It is documented because it is part of the public HTTP surface.

Hamstrack Cloud serves the sink, so the policy header you receive names it. A [self-hosted instance](api-dc.md#browser-violation-reports-csp) does not, unless its operator turned collection on.

**Unauthenticated, by necessity.** A browser sends a violation report with no credentials, so requiring any is requiring a report nobody can send. What makes that safe is that the endpoint can only ingest: there is no read path behind it and nothing it accepts or returns is tenant-derived. The exemption is scoped to `POST` on this one path — every other method stays inside the authenticated `/api/**` set.
...
| `429` | Over the sink's own volume budget — 60 reports a minute from one client address, 600 across the instance. `Retry-After` in seconds, never above 60, and no body. It is not the auth budget and never spends it: see [Rate limits](#rate-limits) |
| `405` | This instance does not serve the sink. You will not meet it on Cloud; a self-hosted instance answers it unless its operator turned collection on. Serving the endpoint and naming it in the policy header are one setting, so a `report-uri` pointing at a path the instance does not serve cannot be configured |

**What an accepted report becomes.** One log line on the instance and two counters — no table, no row, no file, and nothing readable back through this API or any other. The URLs inside it are reduced before they are logged: the reporting page keeps its path and loses its query, because in this product that query can be an [HQL search](#search-hql) — the text of somebody's issue titles — and a report about a page this instance does not serve is dropped rather than logged, which is what stops a public sink being anybody's journal.
