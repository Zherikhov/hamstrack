...
| Method | Path | Auth | Description |
|---|---|---|---|
| `POST` | `/auth/register` | — | Create an account; sends a verification email. `201`. `409` when the address is already registered (addresses are matched case-insensitively — see below). `422` when `password` cannot be stored as sent — [a value this project published](#a-password-this-project-published--a-422), or one [longer than 72 UTF-8 bytes](#a-password-longer-than-72-utf-8-bytes--a-422) |
