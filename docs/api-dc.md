...
| `422` | Semantically invalid reference (unknown status/type/assignee, a user who [may not be assigned](#assignability--a-422-not-a-403) in this project, workflow-forbidden transition, an [unusable role reference](#unknown-role--a-422), an [unknown or wrong-scope permission](#scope-is-immutable-and-permissions-must-match-it)) or a request that names a role with [neither or both](#naming-a-role-roleid-and-the-deprecated-role-key) of `roleId` / `role`. **Also**, on `register` and `reset-password` only, a `password` / `newPassword` that cannot be stored as sent — either [a value this project published](#a-password-this-project-published--a-422) or one [longer than 72 UTF-8 bytes](#a-password-longer-than-72-utf-8-bytes--a-422) — and, on the [custom-field upsert](#system-administration), a `config` past its document, option-count or option-text ceiling. **Plus** the app-wide [time-budget refusal](#errors) (`errorType: "STATEMENT_BUDGET_EXCEEDED"`), which any request can answer and which carries no `Retry-After` — do not retry it |
