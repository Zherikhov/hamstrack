...
| `RATE_LIMIT_ENABLED` | `true` | Master switch for every limiter that **has** an off switch — which is more than just the auth ones, and is not quite every limiter in the app: the per-IP auth budgets and login backoff, the per-principal reports budget (`REPORTS_REQUESTS_PER_MINUTE`), the per-principal search budget (`SEARCH_REQUESTS_PER_MINUTE`) the [invitation ceilings](#invitations-are-throttled) (the `INVITE_*` settings below, including the two counted in the database) and the [per-address ceilings on reset and verification mail](#rate-limits) (the `AUTH_MAIL_*` settings below, counted the same way). Setting it `false` disables all of those — and note what that means where the refusal is invisible: with the switch off, `POST /auth/forgot-password` sends a link on every request, from anybody, at any rate, and nothing anywhere says so — and it is the only way to turn any of them off, since none of the individual settings accepts an "unlimited" value (see [Rate limits](#rate-limits)). **One throttle is deliberately outside it:** the [backlog-rebalance cooldown](#rate-limits) on `POST …/issues/{number}/rank` is a fixed internal safety valve protecting the database from a whole-table rewrite storm, and it has no environment variable of its own and no off switch — so that endpoint still answers `429` on an instance where this is `false`. The mail-send history sweep behind the invitation ceilings keeps running while the switch is off, so nothing grows unbounded |
...
**Inviting somebody is throttled too, and its `429` behaves differently from the budgets above.** [`POST …/workspaces/{id}/invites`](#invitations-are-throttled) carries three ceilings — a cooldown before the same sender may invite the same address again (60 minutes by default, across every workspace on the instance), a cap on how many invitations one address may receive per day from the whole instance (5), and a per-sender volume budget (20 an hour and 100 a day) — and answers `429` + `Retry-After` past any of them, with nothing in the status to say which. A refusal sends no mail and writes no invitation, so the identical request is safe to repeat after the wait. Unlike the two budgets, these are spent **after** the workspace is resolved, so this `429` never precedes a `404`: see [Invitations are throttled](#invitations-are-throttled). The five settings behind them are the `INVITE_*` variables below, and `RATE_LIMIT_ENABLED=false` switches all three off along with every other limiter **that has an off switch** — the rank-rebalance cooldown above is the one that has none.
