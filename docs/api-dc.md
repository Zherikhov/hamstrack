...
| `GET/POST` | `/admin/fields` | List custom fields (with usage counts) / create (`{"name", "key?", "type", "config?", "description?"}`; blank `key` is derived from the name — key and type are immutable afterwards). Create/rename returns `409` when the name **or** key collides with any field visible to the scope, inherited ones included (reuse it instead of duplicating). **Create** has a second `409`: a key the [HQL search vocabulary](#search-hql) has claimed — a name the vocabulary itself registers, its spelling aliases included — is refused, because a search name always outranks a custom field of the same key, so such a field would work everywhere in the product except search, where `/search/schema` would silently omit it. The claimed set is narrower than the set of names a query resolves: a [retired key](#search-hql) resolves to the field that replaced it without being claimed here, so a custom field keyed with one is created normally and then outranks that fallback — unless the retired key is also a registered spelling, which is reserved like any other. The check runs **after** the blank-`key` derivation, so a field merely *named* "Project" is refused just as `"key": "project"` is; it never applies on update (a key is immutable there) and it is never retroactive |
...
- **Reuse over duplication** — creating or renaming a status / priority / issue type / field (fields also on `key`) is refused with `409` when the name collides with any item **visible** to the scope, whether it's an own-scope row or one inherited from a wider scope (a system/global item, or a workspace item inherited by a project). Bind the inherited item instead of minting a scoped duplicate. A field's `key` carries one more `409`, identical at every scope: on **create** it may not be a name the [HQL search vocabulary](#search-hql) has claimed, checked after the blank-`key` derivation and never on update — see [System administration](#system-administration).
