...
**This is not a password-strength check, and it is not a denylist.** It holds only values *we* printed, under their own variable's name. A weak password this project never published is accepted exactly as it was before: the length and composition rules on both endpoints are unchanged (8–100 characters, ordinary [validation](#validation-failures-400) otherwise). Do not surface it as "your password is too weak" — the accurate sentence is the one in `detail`: that particular value is public, so choose another.
