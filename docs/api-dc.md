...
Every per-principal budget is checked **before** the workspace and project are resolved — the only `429`s in this API spent before the resource is resolved, and the [invitation ceilings](#invitations-are-throttled) are the deliberate counter-case — and being keyed on the principal they bound **one user, not one workspace**: aggregate load still scales with member count (see [Reports](#reports)). Beyond the budgets and ceilings named here and the auth endpoints above, nothing else in this API is rate-limited. Note that a budget is not a path: saved filters are throttled without sitting under `…/search/**`, because what earns a throttle is the work a handler does, not where it is mounted.
...
It names projects and their volumes — including projects the caller is not a member of — so it is real disclosure and requires **`workspace.edit`**: `403` naming the permission for a proven member without it, `404` (never `403`) for a non-member or an unknown workspace. And it is a grouped aggregate over every attachment row in the workspace — work proportional to what the tenant holds rather than to what the response returns — so it spends the **[reports budget](#rate-limits)** (`REPORTS_REQUESTS_PER_MINUTE`, 60/min by default) and answers `429` + `Retry-After` past it. The asymmetry with the unbudgeted summary runs the safe way round: a caller refused here who falls back to the summary gets *less* information, never a way around the bound.
