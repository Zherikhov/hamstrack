...
**This `429` comes *after* the `404`, which is the opposite of every other throttle in this API.** The per-principal [report and search budgets](#rate-limits) are spent before the workspace is resolved, so a `429` from those says nothing about whether the caller can see the resource. Here the ceilings are spent **last**, after every other check the endpoint makes: the workspace is resolved first (an unknown workspace and one you are not a member of are the same `404`), then [`workspace.member.manage`](#permissions) (`403`), then the role reference and the [grant ceiling](#the-grant-ceiling) (`422` / `403`), then "already a member" (`409`) — and only then the ceilings. **A workspace you are not a member of answers `404` even when you are over every ceiling**, so a `429` here is only ever seen by a member who may invite. The inversion is deliberate: a refusal keyed on a *recipient* spent before the tenancy check would answer, to a non-member, a question about invitation traffic elsewhere on the instance. So the rule that holds for those budgets — that a `429` says nothing about whether you can see the resource — is exactly the rule that does not hold here. One corollary worth relying on: no refusal above the ceilings spends any of your allowance, so being refused for a role or a workspace costs you nothing.
