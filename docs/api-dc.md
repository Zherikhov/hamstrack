...
| `GET/POST` | `/admin/fields` | List custom fields (with usage counts) / create (`{"name", "key?", "type", "config?", "description?"}`; blank `key` is derived from the name — key and type are immutable afterwards). Create/rename returns `409` when the name **or** key collides with any field visible to the scope, inherited ones included (reuse it instead of duplicating). **Create** has a second `409`: a key the [HQL search vocabulary](#search-hql) has claimed — any name a query resolves to a built-in field, aliases included — is refused, because a search name always outranks a custom field of the same key, so such a field would work everywhere in the product except search, where `/search/schema` would silently omit it. The check runs **after** the blank-`key` derivation, so a field merely *named* "Project" is refused just as `"key": "project"` is; it never applies on update (a key is immutable there) and it is never retroactive |
