...
- [Browser violation reports (CSP)](#browser-violation-reports-csp)
...
| `AGILE_SECTION_MAX_ISSUES` (`app.agile.section-max-issues`) | `300` | Per-section cap on every [planning read](#sprints--backlog) — the whole view (`GET …/backlog`) and any one of its sections on its own (`…/backlog/sections/…`), which is bounded by this and takes no page size of its own. A section holding more reports `truncated: true` with the full count in `totalAvailable`, while its `stats` still cover the whole section. Valid range 1–2000, but **not independently attainable** — see the joint bound on `AGILE_MAX_OPEN_SPRINTS` (at the default 20 open sprints this effectively caps out around 952). An out-of-range value aborts startup instead of being clamped. **The unit is rows and the cost is megabytes, with a concurrency multiplier on top** — an assembled issue is ~1.9 KB of transient heap, so the default view is ~12 MB per request and the joint ceiling ~38 MB, against the 512 MB a default install gets (it follows `APP_MEMORY_LIMIT`, it is not a constant). Planning reads take permits from `EXPENSIVE_READ_MAX_IN_FLIGHT`, so that many such views are alive at once — **shared with reports and search rather than added to them**. Raising this raises a per-request heap figure that is then multiplied by the occupancy ceiling |
...
| `REPORTS_MAX_ROWS` (`app.reports.max-rows`) | `20000` | Most issue **rows** one [report](#reports) may materialise before it declares itself truncated (`POST …/search/insights` is outside its reach entirely — that report aggregates in SQL and materialises no issue row, so it always answers `truncated: false` and discloses its own bucket caps, which are fixed constants and not configurable, through `slicesTruncated` / `cellsTruncated`) — echoed to every client as `meta.cap`, with `meta.truncated: true` when it bites. It does **not** bite on `GET …/reports/flow`, which aggregates in PostgreSQL and returns at most one row per bucket (that report always answers `truncated: false`); the value is still echoed there so a client never has to guess which budget a number was measured against. It is the real budget for the row-level reports: `GET …/reports/cycle-time` ships the **most recent** rows up to it and `GET …/reports/aging` the **oldest** — each keeps the end its reader came for. On the two sprint reports it caps **ledger event rows** rather than issues, and the surviving end is the **oldest**, so a truncated sprint keeps its commitment and its earliest changes (the head of the chart stays exact and its tail goes missing — the recoverable failure; the opposite ordering would lose the commitment batch and make every number wrong). It effectively cannot bite there: one sprint's ledger is hundreds of rows. `GET …/reports/velocity` reads up to twelve sprints' ledgers in one sweep against the same cap, and there it is the one setting in this table that can change what a report is willing to say: once it bites, **every** sampled sprint whose rows fell past it is dropped from the chart (drawing them as all-zero bars would feed those zeros into the forecast and understate the team), **and the forecast band is suppressed outright** — a band over whichever bars happened to fit is not the one the caller asked for. Lowering this therefore shortens a velocity chart *and* withholds its band, rather than merely trimming a list; `meta.truncated` is the signal in every case, and `meta.basedOnIssues` is counted in the database above the cap on all three sprint-ledger reports, so it still states how many issues the report was about. Do not confuse it with the burn-up's `seriesTruncatedAt`, which is the **day** clip governed by `REPORTS_MAX_WINDOW_DAYS` above and deliberately does **not** set `meta.truncated` — two limits, two signals. Lowering it shortens those lists and **never moves a number**: the percentiles and counts on both are aggregates computed over the whole matching set, not over the shipped rows, so a truncated report still describes the full population and says so in `meta.basedOnIssues` / `sampleSize`. Identical on DC and Cloud, same reasoning as above. Valid range 1–50000 — an out-of-range value aborts startup instead of being clamped. The ceiling is a byte budget wearing a row count: a shipped row costs roughly 1.9 KB of transient heap at worst (the JDBC row, the DTO and the buffered JSON are all alive at once), so 50000 is ~95 MB for a single request and the old 200000 was ~380 MB, i.e. a documented value that could OOM the instance in one GET. The ceiling is sized against a **reference heap of 512 MB**, and since `HD-152` the deployment supplies that heap rather than assuming it: the published image runs the JVM at `-XX:MaxRAMPercentage=50` and the bundled compose caps the app container at `1g` (`APP_MEMORY_LIMIT`), which is 512 MB of heap exactly, so the arithmetic above is measured against the heap a default install really has. It is true of a DEFAULT install and not of every install: the heap follows the **container** limit, so `APP_MEMORY_LIMIT=2g` makes it 1 GB and this ceiling conservative, an operator at a 4 GB container limit or more is told to pin an explicit `-Xmx` instead (the 50% split over-reserves at a large limit, since most non-heap cost is constant), and running the image with no container limit at all takes the percentage against host RAM. So read 512 MB as the figure this arithmetic is stated against rather than as a guarantee. Nothing bounds concurrency either, so ~95 MB is per request and never a total. Sizing guidance, and the note for operators upgrading from before 0.17.0 on a host over 2 GB (where the default is LESS heap than they had), are in `docs/self-hosting.md` |
| `REPORTS_REQUESTS_PER_MINUTE` (`app.reports.requests-per-minute`) | `60` | How many report requests **one user** may make per minute across the whole `…/reports/**` surface **plus `POST …/search/insights`**, which is a report that does not live under that path (its dataset is an HQL query, so it hangs off `/search`) and is therefore bound to the limiter explicitly rather than by prefix — all seven reports share this one budget rather than holding one each, so a page opening the sprint burn-up and the sprint review together spends two of it, and an Insights panel refresh spends one from the same pot. Past it the answer is `429` + `Retry-After` (seconds) — a retryable throttle, never a narrowed or approximated report. The budget is spent **before** the workspace and project are resolved, so an over-budget caller gets `429` even for a project they cannot see; it is keyed on the caller and never on the project, which is what keeps that refusal free of tenancy information and stops one colleague's open dashboard from throttling their whole team. For the six project-scoped reports this is the only thing bounding the **work** a report does — `REPORTS_MAX_WINDOW_DAYS` bounds the response array, not the cost of producing it (the opening balance is O(project history)), and `Cache-Control: private` means no shared cache ever absorbs a repeat. Counters are in-memory per app node, so a multi-instance deployment gets one budget per instance; `RATE_LIMIT_ENABLED=false` switches it off along with every other limiter **that has an off switch** — the backlog-rebalance cooldown on `POST …/issues/{number}/rank` has none and keeps running (see `RATE_LIMIT_ENABLED` below). Valid range 1–10000 — there is deliberately no "unlimited", and an out-of-range value aborts startup instead of being clamped. Identical on DC and Cloud with no profile override, same reasoning as the two settings above. The Insights panel is the one exception to "only thing": it is additionally inside `SEARCH_REQUESTS_PER_MINUTE` below. It sits on the reports limiter deliberately, because removing that binding would raise the panel’s allowance to the search budget as a side effect — so **the lower configured value binds**, and lowering *either* property lowers the panel. |
| `SEARCH_REQUESTS_PER_MINUTE` (`app.search.requests-per-minute`) | `120` | How many **search-surface** requests **one user** may make per minute across the whole `…/workspaces/*/search/**` path: [`POST …/search`](#search-hql), `GET …/search/schema`, `GET …/search/suggest` and [`POST …/search/insights`](#insights--break-down-the-query-in-the-search-box) **and the whole `…/workspaces/*/filters/**` path** — every saved-filter operation, `GET` and `DELETE` included, since the binding is by path and not by method. Saved filters are on this budget because **HQL validation is search-surface work wherever it is mounted**: validating a filter builds the same resolution context `…/search/schema` pays for (roughly eight statements, including a workspace-wide label projection and a full member scan), so creating one with a deliberately invalid body was an unthrottled eight-query refusal loop. It is charged here rather than to the reports pot because a saved filter *is* a saved search. Saved filters (`…/workspaces/*/filters/**`) are on this budget too: validating a filter's HQL builds the same resolution context `…/search/schema` pays for (a workspace-wide label projection and a full member scan), so an invalid-body loop there was the same unthrottled cost wearing different clothes — and a saved filter is a saved search, done by the same person. Past it the answer is `429` + `Retry-After` (seconds) — a retryable throttle, never a narrowed result. **Its own budget rather than the reports one** because a person typing in a search box legitimately fires several requests a minute and must not be starved to protect charts; 120 is roughly ten times ordinary SPA use. Search is not the cheap surface it looks like: a query may carry up to 50 leaf predicates, a `text ~` leaf compiles to two unanchored, unindexable `LIKE`s over a TEXT column, and the endpoint runs the whole predicate **twice** per request (count, then page) — so until this existed the expensive door was the unthrottled one. Spent **before** the workspace is resolved, so an over-budget caller is refused even for a workspace they cannot see. Counters are in-memory per app node, so a multi-instance deployment gets one budget per instance. Valid range 1–10000 — there is deliberately no "unlimited": `0` is out of range and fails startup, and the off switch is `RATE_LIMIT_ENABLED`, which disables every limiter that **has** an off switch (the backlog-rebalance cooldown has none — see `RATE_LIMIT_ENABLED` below). Identical on DC and Cloud with no profile override. **Leave the line out to get the default — `SEARCH_REQUESTS_PER_MINUTE=` is an empty value, not an absent one, and it stops the boot rather than restoring 120**. The Insights panel is inside **both** this budget and `REPORTS_REQUESTS_PER_MINUTE` above. It sits on the reports limiter deliberately, because removing that binding would raise the panel’s allowance to the search budget as a side effect — so **the lower configured value binds**, and lowering *either* property lowers the panel. |
| `PLANNING_REQUESTS_PER_MINUTE` (`app.planning.requests-per-minute`) | `240` | How many **planning** requests **one user** may make per minute across the whole `…/workspaces/*/projects/*/backlog/**` path — the whole view [`GET …/backlog`](#the-planning-view) and [every section read](#refreshing-one-section) under `…/backlog/sections/…`. One pattern over the surface rather than a list of endpoints, so a planning read added later is budgeted the moment it exists rather than when somebody remembers. Past it the answer is `429` + `Retry-After` (seconds) — a retryable throttle, never a narrowed section, a smaller `sectionCap` or a `truncated` that was not earned. **Its own pot rather than the reports one**: a card dragged across sections costs two section refreshes, so an ordinary grooming session would reach 60/min inside three minutes, and a `429` in the middle of a drag reads as a product defect where a `429` on a chart reads as a delay. **Where 240 comes from, so you can move it on evidence rather than on feel**: the SPA's refresh loop is *sequential* — a drag across sections issues its two section fetches one after the other — so a very busy tab peaks at roughly 70 planning requests a minute (about 30 drags, mostly cross-section, plus about 10 whole-view fetches from filter toggles and sprint mutations), and the same person with a second project open in a second tab at roughly 140. 240 is ~3.4x the busy single tab and ~1.7x that two-tab case: sized to catch a client in a loop, not to shape ordinary use. **It is the cheaper half of the guard on this surface** — what protects your connection pool is `EXPENSIVE_READ_MAX_IN_FLIGHT` below, which the planning reads also spend, because a rate spends the same unit whether a request takes 8 ms or 8 s and its protection therefore evaporates exactly as the instance slows down. Spent **before** the workspace and project are resolved and keyed on the caller, so an over-budget caller gets `429` even for a project they cannot see, and the refusal carries no tenancy information. Counters are in-memory per app node, so a multi-instance deployment gets one budget per instance — a bound on abuse, never a quota to quote. Valid range 1–10000 — there is deliberately no "unlimited": `0` is out of range and fails startup, and **`PLANNING_REQUESTS_PER_MINUTE=` is an empty value rather than an absent one and stops the boot instead of restoring 240, so leave the line out to get the default**. The off switch is `RATE_LIMIT_ENABLED`, which deliberately leaves the occupancy bound above running. Identical on DC and Cloud with no profile override |
| `EXPENSIVE_READ_LIMIT_ENABLED` (`app.expensive-read.limit-enabled`) | `true` | Whether the **occupancy bound** on the expensive-read surface — the three settings below — is enforced. It bounds how many expensive reads may be *running at once*, which the per-minute budgets above do not and never did: a rate spends the same unit whether a request takes 8 ms or 8 s, so its protection evaporates exactly as the instance slows down, while an occupancy bound tightens. **Deliberately outside `RATE_LIMIT_ENABLED`**, for the reason the [storage quota](#workspace-storage) is: removing a bound on your connection pool must not require disabling brute-force protection on your login page, and debugging a rate limiter must not require removing the bulkhead. So `RATE_LIMIT_ENABLED=false` leaves this one running, and setting *this* `false` leaves nothing between a single caller and every connection on the replica — which is a measured outcome and not a theoretical one: one user, inside every budget they held, saturated an instance |
| `EXPENSIVE_READ_MAX_IN_FLIGHT_PER_PRINCIPAL` (`app.expensive-read.max-in-flight-per-principal`) | `3` | How many requests on the expensive-read surface — `…/reports/**`, `…/search/**` (the [Insights panel](#insights--break-down-the-query-in-the-search-box) included), `…/filters/**`, the [storage breakdown](#workspace-storage) and every [planning read](#the-planning-view) under `…/backlog/**` — **one user** may have running at once. Past it the answer is `429` with `errorType: "TOO_MANY_IN_FLIGHT"` and `Retry-After: 1` — a refusal about that caller's own conduct, cleared by one of their own requests finishing (see [Rate limits](#rate-limits)). `3` is one above the largest concurrent burst a correct client makes here — the search results page mounts three of these in parallel, and a page that refuses its own load is a bug however correct the arithmetic — and it is 30 % of the default pool of 10 for any one caller, against an entitlement that used to be the whole pool. Putting the planning reads on this surface did **not** raise that burst and so did not reopen this number: the Backlog page puts one request on it, and its section refreshes run one after the other rather than in parallel. A change that parallelised them would have to re-argue this row. An explicit value must be **≤ `EXPENSIVE_READ_MAX_IN_FLIGHT`** or the boot fails naming both: above it this number can never be reached, and the caller would meet the wrong refusal with the wrong remedy. **While you set neither, both are derived from `DB_POOL_MAX_SIZE`** — this one is `3` clamped to the derived surface share, which is `3` on the default pool of 10 — so an install that has never named these variables boots on any pool size. Valid range 1–100, plus **`-1` for "derive it from the pool"** — the same thing leaving the variable out does, and the way to ask for it where there is no line to comment out (a systemd `EnvironmentFile`, an ECS task definition, a Kubernetes ConfigMap). No "unlimited": `0` is out of range and an empty value stops the boot rather than restoring the default. **And if you pin this one while the surface ceiling stays derived, it is compared with that derived share and not with the pool** — on `DB_POOL_MAX_SIZE=4` the derived share is `2`, so a pinned `3` is narrowed to `2` with a WARN rather than refusing the boot; the refusal is reserved for the case where you pinned both |
| `EXPENSIVE_READ_MAX_IN_FLIGHT` (`app.expensive-read.max-in-flight`) | `6` | How many requests on that surface may be running at once **across every user**. This is the number that reserves the pool for everything else: the rest of the API always keeps `DB_POOL_MAX_SIZE` minus this many connections, however many callers ask. Past it the answer is `429` with `errorType: "EXPENSIVE_SURFACE_BUSY"` and `Retry-After: 1`, and the refused caller may hold none of the places — the refusal is about the instance, not about them, so it never tells them to reduce their own concurrency. **The relation to the pool is checked at startup rather than described in prose**: an explicit value must be strictly below the pool's actual maximum or the boot fails (at or above it the expensive surface can hold every connection, and a bulkhead that reserves nothing is not a bulkhead), and an explicit value above 60 % of the pool logs one WARN naming how many connections the rest of the API is left with — a sizing judgement, not an error, since a large pool and a known traffic mix make it legitimate. So raise `DB_POOL_MAX_SIZE` first and this after, never the other way round. **Leave it unset and it is derived from the pool instead** — 60 % of `DB_POOL_MAX_SIZE`, capped at the `6` in this column, which is exactly `6` on the default pool of 10 — so an install that has never named it upgrades cleanly whatever its pool size, and the derived value satisfies both rules by construction. A value you *type* is absolute rather than a share on purpose: a share would track the pool and silently change this surface's capacity whenever somebody tuned the pool for an unrelated reason (`work_mem` arithmetic, `max_connections`). Deriving the default is the opposite case — it applies only while you have expressed no intent, it is computed once at startup, and the boot log names the numbers it chose. Counted per app node — for an occupancy bound that is exact rather than weakened, because the pool is per node too, and the ratio holds on every replica without coordination; what does *not* scale that way is the database, where the instance's aggregate is this × replicas and is the number to check against `max_connections` when scaling out. Valid range 1–1000, plus **`-1` for "derive it from the pool"** (identical to leaving the variable unset, and the only way to say it in an environment assembled from a ConfigMap or a task definition). No "unlimited", and an empty value stops the boot |
| `EXPENSIVE_READ_ACQUIRE_WAIT_MS` (`app.expensive-read.acquire-wait-ms`) | `1000` | How long a request may **wait** for a place before it is refused, in milliseconds. `0` means refuse immediately and is the one number in this block that legitimately accepts zero. A waiting request holds no connection and no heap — strictly cheaper than what it replaces, which held both — and it leaves on its own when the bound expires, so waiters are self-limiting. The wait is what lets the per-user number stay tight without refusing a legitimate page load: a client's parallel mount serialises instead of being refused. It bounds the **whole** acquisition rather than each half, so a refusal never takes longer than this plus scheduling. Lower it to shed load faster, raise it to trade latency for fewer refusals. Valid range 0–2000, and that ceiling is the one relation this number has: a waiting request holds a **Tomcat worker thread** rather than a connection, so a longer wait multiplies the thread cost of every refusal — and since this bound stays on when `RATE_LIMIT_ENABLED` is `false`, arrivals past it are unbounded |
| `WRITE_REQUESTS_PER_MINUTE` (`app.write.requests-per-minute`) | `180` | How many **mutating** requests **one user** may make per minute across `…/workspaces/*/projects/*/issues/**` — issue create/update/delete, comment create/update/delete, attachment upload/delete and [backlog rank](#ranking-an-issue). The budget is **method-conditioned**: `GET` on the same paths costs nothing, and `PATCH`/`DELETE` count exactly like `POST`, because a budget that stopped only creation would move a refused client onto the update, and an update is not the cheaper request — it writes history rows, bumps the version and fans out notifications and [SSE](#real-time-events-sse). Past it the answer is `429` + `Retry-After` (seconds) — a retryable throttle, never a partial write. Spent **before** the workspace and project are resolved and keyed on the caller, so an over-budget caller gets `429` even for a project they cannot see, and the refusal carries no tenancy information. Counters are in-memory per app node, so a multi-instance deployment gets one budget per instance; `RATE_LIMIT_ENABLED=false` switches it off along with every other limiter that has an off switch. 180 is 3/s sustained, sized against the SPA's inline-edit saves and board drags. Valid range 1–10000 — no "unlimited", and an out-of-range or empty value aborts startup instead of being clamped or reverting to the default. Identical on DC and Cloud with no profile override |
| `WRITE_UPLOAD_BYTES_PER_MINUTE` (`app.write.upload-bytes-per-minute`) | `250MB` | How many uploaded **bytes** one user may push per minute, summed over the **parsed** sizes of their [attachments](#attachments) — never a client-declared `Content-Length`. A request count does not bound bytes, and the storage quota below does not see churn: upload → delete → upload leaves a workspace's total exactly where it started while every stored byte and every store request in between has been paid for, so this is the only setting that costs that loop anything. Past it the answer is `429` + `Retry-After` with `Too many uploaded bytes` in `detail`, which is how a client tells it from the request budget above. Must be **≥ `ATTACHMENT_MAX_FILE_SIZE`**, or no legal file could ever be uploaded — checked at startup rather than discovered by the first upload after a deploy. In-memory per app node; off switch `RATE_LIMIT_ENABLED`. Identical on DC and Cloud |
| `STORAGE_QUOTA_ENABLED` (`app.storage.quota.enabled`) | `true` | Whether the [workspace attachment-storage ceiling](#workspace-storage) is **enforced**. `ATTACHMENT_MAX_FILE_SIZE` refuses one large file; nothing refused the ten-thousandth small one until this existed. **This is not under `RATE_LIMIT_ENABLED`, deliberately** — see that row below. Setting it `false` disables the **refusal**, not the bookkeeping: usage is still counted, still returned by [`GET …/storage`](#workspace-storage) and still reconciled, and the ceiling-derived fields of that response become `null` so clients narrow on `quotaEnabled` rather than on a sentinel. Bookkeeping keeps running on purpose: an instance that turns the quota back on must not resume from a blank number, and the figure an operator needs in order to choose a ceiling is the one that says what is already in use |
| `STORAGE_QUOTA_WORKSPACE_BYTES` (`app.storage.quota.workspace-bytes`) | `100GB` self-hosted · lower on Cloud (`10GB`) | The ceiling itself, per workspace. **The default differs by deployment model and by property only, never by code path** — same enforcement, same table, same `409` in both, because what each model protects differs: self-hosted, the operator owns the disk and signup is closed by default, so the number is a safety net against runaway growth; on Cloud signup is public and the store bills per byte **and** per request. Past it an upload answers `409` `STORAGE_QUOTA_EXCEEDED` with the figures in the body and **no `Retry-After`** — waiting frees no bytes. It bounds **growth** only: nothing here deletes, archives or expires an attachment, reads and downloads are never quota-gated, and setting it below what a workspace already holds refuses new uploads while leaving every existing file readable and downloadable. Must be ≥ `ATTACHMENT_MAX_FILE_SIZE` (startup check). Takes effect on the next request — nothing is cached. Unlike the per-principal budgets, this bound is counted in the database, so it is **exact and instance-wide**: several app nodes do not multiply it and a redeploy does not re-arm it |
| `STORAGE_QUOTA_WARN_PERCENT` (`app.storage.quota.warn-at-percent`) | `80` | The share of the quota at which the app starts **saying so** — returned as `warnAtPercent` by [`GET …/storage`](#workspace-storage) and rendered beside the upload control. It changes **no** server behaviour: nothing is refused, narrowed or slowed at this level, and no status code depends on it. It exists so that the first thing a workspace hears about its ceiling is not a refusal. Valid range 1–99 |
| `STORAGE_QUOTA_RECONCILE_CRON` (`app.storage.quota.reconcile-cron`) | `0 20 3 * * *` | When to re-check that the stored usage counter still equals the attachment rows it claims to count. It changes no API contract, only the freshness of the figures [`GET …/storage`](#workspace-storage) reports and of `unattributedBytes` on the breakdown. An **empty** value disables the schedule, which is a legitimate operator choice and is not silent — the drift metric then ages and its staleness alert fires. An **invalid** cron expression still fails the boot, because a typo is not a decision |
| `ROLES_MAX_CUSTOM_PER_WORKSPACE` (`app.roles.max-custom-per-workspace`) | `50` | Max [custom roles](#custom-roles) one workspace may define, counted across **both** scopes with the built-in templates excluded (they belong to no workspace). Duplicating past it returns `409` `errorType: "ROLE_LIMIT_REACHED"`. It is a **sprawl guard, never a licence check** — custom roles are a product feature and not a plan feature, so the value is identical on DC and Cloud and there is no second code path anywhere. If you want to limit them, lower this number. Valid range 1–500 — an out-of-range value aborts startup instead of being clamped. The count is taken under a row lock on the workspace, so the cap is exact rather than advisory, and taking that lock makes a duplicate likelier than most calls to lose a lock race (a retryable `409` with `Retry-After`; the bound it waits against is this one, applied to every transaction — see `DB_LOCK_TIMEOUT_MS` below) |
...
| `DB_LOCK_TIMEOUT_MS` (`app.locking.lock-timeout-ms`) | `3000` | How long a transaction may wait for a row lock before giving up, in milliseconds. **From 0.17.0 it is applied to every transaction the application opens**, not only to the [membership mutations](#managing-members) that lock deliberately — so any contended write can now answer the retryable `409` below rather than waiting indefinitely. It is issued in the same round trip as `DB_STATEMENT_TIMEOUT_MS`, because `statement_timeout` counts lock-wait time: without a lock bound, a contended write would be cancelled by the statement bound and answered a `422` that forbids the retry that would have succeeded. Exceeding it is a retryable `409` with `Retry-After` (see [Errors](#errors)), never a `500`. Applied with `SET LOCAL` inside each transaction the application opens, never as a server-wide PostgreSQL `lock_timeout`, so Flyway migrations sharing the datasource are unaffected — they run their own transactions and never go through the application's transaction manager. Lower it to shed load faster under contention, raise it if legitimate removals of very busy members time out — **but from 0.17.0 raise `DB_STATEMENT_TIMEOUT_MS` in the same edit**: it must stay at least twice this value, so while it sits at its default `10000` this one may not exceed **5000**, and a higher value here stops the boot naming both properties. Valid range 100–60000 — an out-of-range value aborts startup instead of being clamped (PostgreSQL reads `0` as "wait for ever", which is the setting this exists to prevent) |
| `DB_STATEMENT_TIMEOUT_MS` (`app.persistence.statement-timeout-ms`) | `10000` | How long **any one statement** of an application transaction may run before PostgreSQL cancels it, in milliseconds. Applied with `SET LOCAL` to every transaction the application opens — not to a list of endpoints, because how long a statement takes is a property of how much data the tenant has and not of which feature issued it, so **any** endpoint that touches the database can in principle answer it. Exceeding it is a `422` with `errorType: "STATEMENT_BUDGET_EXCEEDED"` and deliberately **no** `Retry-After`: unlike the `409` above, an identical retry costs identical time (see [Errors](#errors)). Flyway migrations are deliberately outside it — they run their own transactions, and a schema change legitimately takes minutes. Raise it if a large tenant meets it on legitimate work; it must stay at least **2x** `DB_LOCK_TIMEOUT_MS` (PostgreSQL counts lock-wait time inside the statement, so a smaller value would fire first and silently replace the retryable `409` with a non-retryable `422`) and an out-of-range value aborts startup instead of being clamped. **The `2x` rule is the binding constraint, not the range**: at the default `DB_LOCK_TIMEOUT_MS` of `3000` the smallest value that boots is `6000`. **Raising it is not free** — every second added is a second one request may hold one of `DB_POOL_MAX_SIZE` connections, so the same pool serves fewer concurrent slow requests; past ~30 s raise the pool with it. It bounds a *statement*, never how long a connection is held. Identical in `dc` and `cloud` — a deployment that wants a different bound sets the environment variable |
| `DB_CONNECTION_TIMEOUT_MS` (`spring.datasource.hikari.connection-timeout`) | `3000` | How long a request may wait for a **connection from the pool** before it is refused, in milliseconds — and **new in 0.18.0**: until this release the key was never set, so every acquisition waited HikariCP's 30-second default, which nobody had chosen. It is the third member of the family above and the relation is stated once: the lock bound is how long a transaction waits for a *row*, the statement bound how long one statement *runs*, and this is how long a request waits for a *connection*. It bounds **getting** a connection and never how long one is **held** — that is `EXPENSIVE_READ_MAX_IN_FLIGHT` above. Exceeding it answers **`503`** with `errorType: "DATABASE_BUSY"` and `Retry-After: 1`, on **any** endpoint including unauthenticated ones — a 5xx on purpose, unlike the `422` above, because the obstacle is somebody else's transaction, which ends, and one retry costs one acquisition attempt (see [Errors](#errors)). **On a busy or under-provisioned instance this turns a slow period into visible errors rather than a slow one**, which is the trade this bound makes: the usual remedy is a larger `DB_POOL_MAX_SIZE`, since raising this buys waiting rather than capacity. Flyway shares the datasource and is unaffected — migrations take their connections at startup from a pool nothing else is using yet, and this bounds acquisition rather than duration. **It sets two HikariCP values, not one**: the same number is also the pool's `validation-timeout`, which bounds the aliveness check run on a connection that has sat idle in the pool past `aliveBypassWindowMs` — so raising this to `6000` gives that check 6 seconds too, and a single acquisition of a stale connection can cost the bound twice. They are one number deliberately: left apart, one `getConnection()` costs this bound *plus* HikariCP's 5-second validation default, in exactly the degraded-database case this bound exists for. The app compares **both** values against what the pool ended up holding and refuses to start if either was **moved by another name or from another property source** — what it checks is the number actually in force, whatever moved it, so no list of ways to move it is part of the rule. The likeliest is a dashes-removed environment spelling, `SPRING_DATASOURCE_HIKARI_CONNECTIONTIMEOUT` or `SPRING_DATASOURCE_HIKARI_VALIDATIONTIMEOUT`, which Boot's relaxed binding accepts and which win over the app's own file; the refusal names those first and then a `DataSourceProperties` customiser or a second property source as the other places to look. Set the value through `DB_CONNECTION_TIMEOUT_MS` and nothing else. Must stay at or above `DB_LOCK_TIMEOUT_MS` or every boot logs a sizing WARN: a request that gives up before a lock holder is entitled to let go turns ordinary row contention — a retryable `409` — into pool refusals. **`0` is refused at startup**, because HikariCP maps it to `Integer.MAX_VALUE` (about 24.8 days), so it means *no bound* rather than *no wait*; so is anything below **250** (HikariCP's own floor) and a **blank** value — comment the line out, never blank it. **And there is a ceiling nothing else in this table implies**: the shutdown residue write must fit inside `APP_STOP_GRACE_SECONDS` together with the mail drain, so at the shipped mail defaults (`MAIL_ASYNC_SHUTDOWN_DRAIN_SECONDS=15`, `MAIL_ASYNC_QUEUE_CAPACITY=100`, `APP_STOP_GRACE_SECONDS=30`) the largest value that boots is **13900** — restoring the old 30 s stops the boot, with a refusal naming all four knobs. Identical in `dc` and `cloud` — a deployment that wants a different bound sets the environment variable |
| `CSP_REPORT_ONLY_ENABLED` (`app.csp.report-only-enabled`) | `true` | Whether every response carries the `Content-Security-Policy-Report-Only` header. The policy is **report-only in every configuration** — nothing in this release enforces one and no setting makes it — so this changes no status code and blocks nothing; turning it off only removes the header, and with it the evidence a violation leaves in a browser's console |
| `CSP_POLICY` (`app.csp.policy`) | empty | Replaces the built-in directive string wholesale. The built-in policy names no address of yours, so leave this empty unless you have a reason. **Checked at startup**: at most 2000 characters, printable ASCII only — a value carrying a carriage return or line feed would split every response this instance writes, so a bad one aborts the boot instead of shipping. Also refused: a `report-uri` **aimed at this instance** (one with no host, or with your `APP_BASE_URL` host) that does not resolve to the path this instance serves with the sink on — matched the way the mapping matches, so a different case, a percent-encoded character or a trailing slash is caught rather than silently answering `405` to every browser — and so is an encoded slash (`%2F`), which decodes to that path while the server rejects it with `400`. One naming a collector on any other host, including another Hamstrack collecting for a fleet, is untouched |
| `CSP_REPORT_SINK_ENABLED` (`app.csp.sink-enabled`) | `false` (`true` on Cloud) | Whether this instance **serves** [`POST /security/csp-report`](#browser-violation-reports-csp) — and, by the same setting, whether the policy header names a `report-uri`. Off by default, so on a default install that endpoint is absent and the path answers `405`. Turning it on publishes an unauthenticated endpoint: read the section before you do, and pair it with `RATE_LIMIT_TRUST_FORWARDED_FOR` so the per-address budget below keys on a real client address rather than on your proxy. With it on, `APP_BASE_URL` must be a URL a host can be read from (scheme included, no underscore in the hostname) or **the app refuses to start** — that host is what decides whose pages a report may be about, and a value with no host would accept everybody's |
| `CSP_REPORTS_PER_MINUTE_PER_IP` (`app.csp.reports-per-minute-per-ip`) | `60` | How many reports **one client address** may send per minute; past it `429` + `Retry-After`, with no body. Valid range 1–10000, and there is no "unlimited" value |
| `CSP_REPORTS_PER_MINUTE` (`app.csp.reports-per-minute`) | `600` | The instance-wide ceiling, 10× the per-address one; valid range 1–100000. The per-address window is charged first, so that is the refusal a sender normally meets. Both are counted in memory **per app node** — N replicas allow up to N × both numbers — and both sit **outside `RATE_LIMIT_ENABLED`**: see that row |
| `RATE_LIMIT_ENABLED` | `true` | Master switch for every limiter that **has** an off switch — which is more than just the auth ones, and is not quite every limiter in the app: the per-IP auth budgets and login backoff, the per-principal reports budget (`REPORTS_REQUESTS_PER_MINUTE`), the per-principal search budget (`SEARCH_REQUESTS_PER_MINUTE`), the per-principal planning budget (`PLANNING_REQUESTS_PER_MINUTE`), the per-principal write budget and upload-byte budget (`WRITE_REQUESTS_PER_MINUTE`, `WRITE_UPLOAD_BYTES_PER_MINUTE`), the [invitation ceilings](#invitations-are-throttled) (the `INVITE_*` settings below, including the two counted in the database) and the [per-address ceilings on reset and verification mail](#rate-limits) (the `AUTH_MAIL_*` settings below, counted the same way). Setting it `false` disables all of those — and note what that means where the refusal is invisible: with the switch off, `POST /auth/forgot-password` sends a link on every request, from anybody, at any rate, and nothing anywhere says so — and it is the only way to turn any of them off, since none of the individual settings accepts an "unlimited" value (see [Rate limits](#rate-limits)). **A control that carries its own switch, or carries none at all, is outside this one** — so read the switch that belongs to a bound before assuming this line reaches it. The [backlog-rebalance cooldown](#rate-limits) on `POST …/issues/{number}/rank` is a fixed internal safety valve protecting the database from a whole-table rewrite storm: no environment variable of its own and no off switch, so that endpoint still answers `429` on an instance where this is `false`. The [workspace storage quota](#workspace-storage) is outside it the other way round, having a switch of its own (`STORAGE_QUOTA_ENABLED` above) — deliberately, because folding it in would mean that removing a bound on your disk requires disabling brute-force protection on your login page, and that debugging a rate limiter requires removing the disk bound. `RATE_LIMIT_ENABLED=false` does not stop the quota refusing uploads, and it does not stop the [expensive-read occupancy bound](#rate-limits) refusing reads either — that one carries `EXPENSIVE_READ_LIMIT_ENABLED` above, for the same reason. The [CSP report sink's budget](#browser-violation-reports-csp) (`CSP_REPORTS_PER_MINUTE_PER_IP` / `CSP_REPORTS_PER_MINUTE` above) is outside it as well, for a reason none of the others has: it is the only bound on a door that requires no account at all, so a switch that reached it would make "turn limiting off while I debug something" mean "let anyone on the internet write unlimited lines into this instance's log" — the off switch for that door is the door, `CSP_REPORT_SINK_ENABLED=false`. The mail-send history sweep behind the invitation ceilings keeps running while the switch is off, so nothing grows unbounded |
...
| `INVITE_MAX_PER_SENDER_PER_HOUR` (`app.invites.max-per-sender-per-hour`) | `20` | Invitations **one account** may send per hour, counted in memory per app node. Past it `POST …/invites` answers `429` + `Retry-After` and nothing is sent or written (see [Invitations are throttled](#invitations-are-throttled)). This window bounds a burst; the largest invite a client can make is a single address, so 20 an hour is invisible to honest use. Valid range 1–1000, and an out-of-range value aborts startup instead of being clamped. There is deliberately **no "unlimited" value** — `0` is out of range on every `INVITE_*` setting, an empty value stops the boot rather than restoring the default, and the off switch is `RATE_LIMIT_ENABLED`. Identical in `dc` and `cloud` and deliberately never profile-gated: a DC install with `PUBLIC_SIGNUP_ENABLED=true` has exactly the Cloud abuse profile, so a looser DC default would protect the wrong instance |
| `INVITE_MAX_PER_SENDER_PER_DAY` (`app.invites.max-per-sender-per-day`) | `100` | Invitations **one account** may send per day — the quota control, and **the first number an operator raises**: onboarding 300 people on install day meets it, and the answer is this line for a day rather than a code change. The two windows are independent, not multiples of each other: spending the hourly allowance five times over ends the day, and the daily refusal does not lift when the hour rolls. Same `429` + `Retry-After`, same in-memory per-node counting — so a deployment running several app nodes allows up to N × this — and the same range rules as above. Valid range 1–10000 |
| `INVITE_RECIPIENT_COOLDOWN_MINUTES` (`app.invites.recipient-cooldown-minutes`) | `60` | How long one account must wait before inviting **the same address** again, **across every workspace on the instance**. It counts the destination **inbox** and not the spelling of the address — see [Invitations are throttled](#invitations-are-throttled). Its state lives in the database rather than in memory, so this ceiling is exact and instance-wide and is unaffected by a restart, a redeploy, which node served the request, or the invitee declining the earlier invitation. Past it: `429` + `Retry-After`, whose value is the seconds until the cooldown expires; the `detail` names the address, which is the caller's own past action, and never names a workspace. Valid range 1–1439 — one minute **inside** the fixed 24 h volume window, because a cooldown as wide as the window it sits inside is refused at startup (at equality a single-bucket policy's cap is unreachable, and one rule for every policy was judged cheaper than an exception for this one) — and **not an independent number**: this setting and `INVITE_EVENT_RETENTION_DAYS` below are validated **as a pair** at startup, because the history this ceiling counts is swept on that retention — a retention that does not outlast the window being counted would silently shorten this ceiling to itself, with no error and no log line. What the check compares is the retention against the **wider of** this cooldown and the widest width any per-recipient volume cap is permitted to count over — 24 hours, fixed in code and not settable by any variable — so the retention has to clear a day whatever this is set to, and a new kind of throttled mail cannot outrun the comparison. At today's ranges the smallest legal retention already covers the largest legal cooldown, so raising this one cannot by itself fail the pair — the check survives to catch a widening of either side, and a failing pair aborts the boot naming both properties rather than letting the drift through; the fix is always to raise the retention or lower this. A cooldown is in any case never allowed to reach further back than the daily window below |
| `INVITE_MAX_PER_RECIPIENT_PER_DAY` (`app.invites.max-per-recipient-per-day`) | `5` | Invitations **one address** may receive per day from the whole instance, whoever sends them — keyed on the destination **inbox** rather than the spelling, like the cooldown above. It is invariant under the number of accounts an attacker holds, which is exactly what the sender-keyed ceilings above cannot be. Past it: `429` + `Retry-After` — and its wait is deliberately coarse, as every per-address *volume* cap's in this product is (the auth-mail ones below included): it names the moment the oldest send in the window ages out **rounded up to the next 15-minute boundary**, so `now + Retry-After` is the same absolute instant however many times a caller probes and only that 15-minute bucket is disclosed (see [Invitations are throttled](#invitations-are-throttled)). The `detail` is kept deliberately terse for the same reason. **The accepted cost, stated plainly:** at the sixth invitation an innocent admin learns that this address has received invitations from workspaces they cannot see — one bit, only to a caller who already holds `workspace.member.manage` and already knows the address. If it proves noisy, raise it rather than removing it — but in small steps, because **what one inbox can be sent in a day is quadratic in this number, not linear**: the count is "your own sends one each, every other sender once", so colluding accounts spend their own sends plus one slot apiece and the real per-inbox bound is `floor((cap + 1)² / 4)` — 9 messages a day at `5`, 30 at `10`. Moving 5 → 10 therefore **triples** the ceiling rather than doubling it. Valid range 1–1000 |
| `INVITE_EVENT_RETENTION_DAYS` (`app.invites.event-retention-days`) | `7` | How many days of mail-send history back **every** address-keyed ceiling in the product — the two above, the auth-mail ones below, and any added later; the table is `mail_send_events` and it is not invitation-specific. **No direct API effect**, but its floor is **not an independent number**: it is validated **as a pair** with `INVITE_RECIPIENT_COOLDOWN_MINUTES` above, because a row this sweep has already deleted is a row no ceiling can count — a retention that does not outlast the windows those ceilings measure would silently shorten them to itself. **The comparison is against the wider of the cooldown and a bound, not against the cooldown alone, and not against a list of today's windows**: every per-recipient volume cap counts over a width fixed in code, those widths differ per kind of mail and a new kind adds another, so what the check compares against is the widest width any of them is *permitted* to declare — 24 hours, which no property can change. The retention therefore has to clear a day however low the cooldown is set, and adding a kind of throttled mail cannot silently invalidate the check. That is where the floor of `2` comes from — one day would be *exactly* the daily window, and exact is not cover: two app nodes whose clocks differ by a second are enough for one node's sweep to delete a row the other node's daily count still needs, and the only symptom is a send that should have been refused. A failing pair aborts the boot naming both properties rather than letting that drift, so raise this in the same edit that raises the cooldown, never after it. The default of `7` sits deliberately far above that floor: after a mail-volume alert fires, this history is the only thing that can answer *who* and *which addresses*, which the metrics deliberately cannot — and it is short enough that this is not a durable record of who-emailed-whom. The retention sweep runs **whether or not `RATE_LIMIT_ENABLED` is true**, so the table cannot grow unbounded on an instance that toggles the switch. **It is not the whole lifetime of the table**: rows written for an *anonymous* caller (the auth mailers below, where there is no sender to be forensic about, and which are the only rows an unauthenticated stranger can make you write) have a second, fixed cutoff of **2 days** that is not settable at all, so their real lifetime is `min(2 days, this value)` — at the default of `7` the anonymous cutoff reaches them first, and at `2` the two coincide. Valid range 2–90 |
| `AUTH_MAIL_RECIPIENT_COOLDOWN_MINUTES` (`app.auth-mail.recipient-cooldown-minutes`) | `1` | How long **one address** waits before another password-reset or verification link can be sent to it, **whoever asks** — `POST /auth/forgot-password`, `POST /auth/resend-verification` and `POST /auth/register` are all anonymous, so every request shares one bucket per address and per kind of mail. Same mechanism and same database-backed history as the `INVITE_*` ceilings above, so it is exact and instance-wide and survives a restart, and it counts the destination **inbox** rather than the spelling. **Past it there is no `429` on the first two** — see [Rate limits](#rate-limits): the request answers `200` with its usual sentence and sends nothing, because those endpoints must answer identically for a registered and an unregistered address. Register does answer `429`, because its own `409` already discloses what a refusal would. Valid range 1–14; the top of the range is one minute **inside** the narrowest of the fixed windows of the row below (15 minutes), since a cooldown **as wide as** the window it sits inside already makes that cap unreachable — the first send starts a cooldown running to the far edge of the window — and startup refuses to build such a policy |
| `AUTH_MAIL_MAX_PER_RECIPIENT_PER_WINDOW` (`app.auth-mail.max-per-recipient-per-window`) | `5` | How many of **each** kind of auth mail — reset, verification, registration — one address may receive per **window**, across everybody who asked. Separate buckets per kind, so traffic aimed at one flow cannot suppress the other — and `POST /auth/register` has a **third** bucket of its own rather than sharing verification's, because `resend-verification` spends a slot at an address with no account while sending no mail at all, which made denying a stranger's signup free and silent. **So this is per bucket, there are three, and each bucket is spent over its own window — the bound on one inbox is a sum of rates, not a multiple of this number.** At the default of `5`: 20 reset emails an hour (a quarter-hour window that refills four times), 5 verification, 5 registration — **30 an hour into one inbox**. **The windows are fixed and are not the same width: 15 minutes for reset, an hour for verification, an hour for registration.** Both are narrower than the invitation cap's fixed day, because a ceiling on a *recovery* flow is also a denial of it — its width is how long somebody who fills it keeps a named person from resetting their own password, silently, since there is no `429` to see there. Reset gets the narrower of the two for that reason; verification keeps the hour, because it locks nobody out of an account they already hold and the tighter mail bound is worth more. Raising this loosens the harassment bound and tightens the denial one at the same time; move it in small steps. Valid range 1–1000. On `forgot-password` / `resend-verification` there is no API effect other than the absence of mail — the status, body and headers are identical either way; on `register` it is a `429` with `Retry-After` |
...
Every endpoint requires the `Authorization` header except the ones whose caller could not hold a token in the first place: [Auth endpoints](#auth-endpoints), [Instance metadata](#instance-metadata) and the [browser CSP report sink](#browser-violation-reports-csp) — and that last exemption is scoped to `POST`, so any other method on its path is an ordinary `/api/**` request. A missing or expired token yields `401 Unauthorized`.

## Conventions

- **Format** — request and response bodies are JSON (`Content-Type: application/json`), UTF-8. The only exceptions: attachment upload (`multipart/form-data`), attachment download (binary), the [report CSV exports](#csv-exports--the-chart-as-a-file) (`text/csv; charset=UTF-8`, sent as an attachment), and the [browser CSP report sink](#browser-violation-reports-csp), which takes a report media type and answers with **no body at all** — status and headers are the whole response, refusals included. Error bodies stay `application/problem+json` on every one of the first three, including the CSV paths.
...
- **Colors** — every color this API accepts is a hex literal: `#RRGGBB`, plus `#RRGGBBAA` on the two surfaces that store transparency ([labels](#labels) and a custom field's select options). The three taxonomy catalogs — statuses, priorities, issue types — take `#RRGGBB` only. A color name, a three-digit shorthand, an empty string or anything else is refused; omitting an optional `color` still means "no color" and is always allowed. **The status differs by door, deliberately**: a `color` declared on a request body is the ordinary field-anchored [`400`](#validation-failures-400), while one buried inside a custom field's `config` is a [`422`](#system-administration), because `config` is free-form JSON that no field constraint reaches into and every refusal on that path is a `422`. Branch on where you sent the value, not on one status. **A color is taken exactly as sent** — nothing is trimmed, so a padded `" #0EA5A4 "` is refused like any other non-color, at every door ([why](#system-administration)). **And one shape gets one sentence**, wherever that shape is checked: `Color must be #RRGGBB or #RRGGBBAA` where the alpha form is allowed, `Color must be #RRGGBB` on the three taxonomy catalogs — as an `errors` entry, or quoted inside the option refusal, but always the same words for the same mistake, and always [human-readable text rather than a code](#validation-failures-400). And what is checked is only that the value **is a color** — a stored color is an identity hue and the text drawn on it is derived at render time, so there is no contrast or readability rule here and there will not be one.
...
**Branch on `status` and on `errorType` — never on which optional members a body carries.** RFC 9457 makes `type` and `instance` optional, and this API is deliberately uneven about them: a refusal rendered by the application carries `instance` (the path you asked for) and leaves `type` unset, while the few written before a request reaches the application — a refusal from a filter, such as the per-IP auth `429` or the `503` described below — carry less, because a filter has no serializer behind it and hand-escaping a caller-supplied path into a hand-built document is not a thing worth owning. Nothing that matters changes with them: the `status`, the `detail`, any `errorType` and any `Retry-After` are identical either way. The JSON examples in this reference spell `type` out as `"about:blank"`, which is what RFC 9457 says an absent `type` means, and omit `instance` for brevity — read each one as the members it is about rather than as the exact set a body carries.
...
**An email address is capped at 255 characters as submitted** wherever a request body carries one. The columns that store an address are `VARCHAR(255)` while the format check alone admits roughly 320, so an over-long but well-formed address used to reach the INSERT and come back a `500`; an address over that raw 255-character bound is now an ordinary field-anchored `400` like any other constraint.

**A raw address within that bound can still overflow once folded, and three endpoints refuse that case with a `400` of their own.** `POST /auth/register`, `POST /admin/users` and `POST …/invites` all store the address lower-cased, and lower-casing is not length-preserving — `İ` (U+0130 LATIN CAPITAL LETTER I WITH DOT ABOVE) becomes two characters — so a 255-character raw address can fold to more than 255. `POST /auth/register` and `POST /admin/users` measure the *folded* form before anything else either door does, because neither has a tenancy gate above it. `POST …/invites` measures it later — after the workspace is resolved, `workspace.member.manage` is checked, and the role and its [grant ceiling](#the-grant-ceiling) are checked, so a caller who is not a member, lacks the permission, or names an ungrantable role never reaches this refusal — but it is still above every check that spends an [invitation allowance](#invitations-are-throttled) or reads another party's state: the existing-member check and the [duplicate-invite check](#one-invitation-per-address-409). All three refuse before the write could ever reach the column. Unlike the field-anchored `400` above, this one names no field and carries **no** `errors` map — the whole body has already been read and bound by the time the fold is measured, so there is no single field left to name — and it carries **no** `errorType` either: `errorType: "VALUE_TOO_LONG"` stays reserved for the database backstop this refusal exists to keep those three doors from ever reaching. `detail` states the arithmetic instead of naming a field, e.g. `` `Email address must be at most 255 characters (lower-casing it turned its 255 characters into 319) — use a shorter address.` `` **One endpoint adds a second rule on top of both of these**: `POST …/invites` also requires the address's [local part to be ASCII](#an-invited-address-must-have-an-ascii-local-part-400), because it is the one body whose email decides where a join token is mailed.
...
**A lost row-lock race is a retryable `409`, not a `500`.** **From 0.17.0 every transaction this API opens bounds how long it will wait for a row lock** (3 s by default), so any write that collides with a longer-running change gives up and is told to retry rather than hanging until a client does; exceeding that bound is this same `409`, never a `500`, and so is the other way a race ends — the database rolling one of two overlapping writes back as a deadlock. Writes that take row locks deliberately — membership changes are the main ones — are simply the likeliest to meet it: the bound is the API's, not theirs. Nothing is left half-applied, so the loser is told to try again rather than shown a fault: `409` with `detail: "Someone else is changing this right now — try again in a moment"` and a `Retry-After: 1` header, the same retry shape [rate limiting](#rate-limits) uses. This is app-wide, not a property of one endpoint, and it is the **only** `409` worth retrying automatically: an invariant conflict (a stale `version`, a name in use, the last owner) will answer identically until something changes. Retry the **identical** request after the header's seconds — change nothing about it.
...
**A request that outruns the server’s time budget is a `422`.** Every transaction this API opens is bounded: a database statement that runs longer than the instance’s budget — 10 seconds by default — is cancelled, and the request answers `422` with `errorType: "STATEMENT_BUDGET_EXCEEDED"` and a `detail` naming that budget in plain units (*“This request was stopped after 10 seconds …”*). **This is a property of the API, not of a set of endpoints.** The bound is applied where transactions begin rather than at individual handlers, because how long a request takes is a function of how much a workspace has accumulated and not of which feature was called — so any request that reads or writes can meet it, and one that is comfortably fast in a small workspace can meet it in a large one without a character of it changing. There is no list of paths that return it to code against. Note also that the budget bounds any *one* statement, so a request may spend longer than that overall before it is stopped.

**There is deliberately no `Retry-After`, because an identical retry fails identically.** The same request over the same data costs the same time, so a retry — immediate, backed off, or automatic inside an SDK — spends the whole budget again and is refused again. That is the one line worth coding against: **never retry this one.** What clears it is a *smaller* request — a narrower date range, a tighter filter, fewer rows — or an operator raising the budget. `detail` offers the narrowing only where narrowing is possible, since some requests take no parameter that could narrow anything, and where nothing can be narrowed it still closes the loop rather than trailing off — it says the request is larger than this instance is configured to answer. It deliberately never tells the reader to ask an administrator: one sentence has to hold in both deployment models, and the setting behind the budget is not one every reader's administrator owns.

**One line tells it from the [`409`](#errors) above**, the refusal it most resembles: that one carries `Retry-After` and means *someone else is changing this right now*, so retrying works; this one carries no header and means *this took too long and will take too long again*, so retrying does not. Two refusals that look alike and give opposite advice.

**A connection the instance could not give in time is a `503` with `errorType: "DATABASE_BUSY"`.** Every request that reads or writes needs a database connection from the instance's pool, and the wait for one is bounded — 3 seconds by default. A request that outwaits that bound is refused rather than parked, with `Retry-After: 1`. **This is a property of the API, not of a set of endpoints**: the bound is on *getting* a connection, which every request needs, so any endpoint can answer it — the unauthenticated ones included — and there is no list of paths to code against. The refusal is written wherever the acquisition failed — inside a handler, or earlier in the filter chain, which is where an authenticated request's bearer token is resolved to a user — and the two writers agree on everything a client reads off it: the same `status`, the same `detail`, the same `errorType` and the same `Retry-After`. They differ only in the optional members this section opens with — the one written in the filter omits `instance`, because a filter has no serializer behind it. **The transaction that failed applied nothing**, since with no connection there was no statement; whether the *request* is safe to repeat is a property of the operation rather than of this status, because an operation that commits in more than one step can be refused on a later step with an earlier one already applied. Repeat it where the endpoint says a repeat is safe, and otherwise re-read before retrying. The body says nothing about the pool: no size, no queue depth, no setting name, no count of who else is waiting.

**It is a `5xx` on purpose, for exactly the reason the time-budget refusal above is not.** That one avoids a 5xx because proxies and SDKs retry a 5xx automatically, and an automatic retry there spends the whole budget again. This one is transient by construction — what is in the way is somebody else's transaction, which ends — and one retry costs one attempt to take a connection, so the status that invites an automatic retry is the correct status. **"Retryable" is not "retry from everywhere at once", though:** the moment this arrives is the moment the instance has least room, so the retry should be one request from one place — an intermediary, or a person pressing a button. Hamstrack's own web client deliberately does **not** retry it automatically; it renders `detail` and leaves the retry to the reader. Do the same in anything that runs in many tabs or many workers at once.

**What still cannot be given this answer is a failure with no answer left to change**, which is a property of the mechanism rather than a list of endpoints: a status can only be set while there is still a response to set it on. That excludes a failure raised on an `ASYNC` or `ERROR` dispatch, where the answer has already been decided or already begun; one raised after the response has been committed, such as an attachment download already streaming or an open [event stream](#real-time-events-sse); and one raised outside any request at all, such as a scheduled job or a migration at startup, which has no caller to refuse. Those are exactly the conditions under which *no* refusal in this API can change an answer, this one included. Read `DATABASE_BUSY` as the shape overload takes wherever an answer can still be given, never as a promise that overload always looks like this.

**Telling the "busy" refusals apart.** More than one status in this API means *something is in the way right now*, and they are separated by `errorType` and by the presence of `Retry-After` rather than by the status alone. They ask for different behaviour, so a client that folds them into one either hammers an instance that is already saturated or abandons a request that would have succeeded. Code against the row rather than against how many rows there are — a refusal added later joins this table instead of invalidating a count in it:

| Refusal | What is in the way | What to do |
|---|---|---|
| `409` with `Retry-After` and **no** `errorType` | Another transaction holds a row this write needs — [lock contention](#errors) | Retry the **identical** request after the header's seconds |
| `429` with `errorType: "EXPENSIVE_SURFACE_BUSY"` (or `TOO_MANY_IN_FLIGHT`) | The [expensive-read surface](#rate-limits) is running as much at once as it may — or your own requests are | Let something finish, then retry shortly and **not** automatically. A `429` with **no** `errorType` is a per-minute budget instead, and waiting out its window is the whole remedy |
| `429` with `Retry-After` and **no body whatsoever** | The [browser CSP report sink](#browser-violation-reports-csp) is over its own volume budget | Nothing you would normally write code for: the sender there is a browser, which reads the status and drops the report. It is never the auth budget and never a per-principal one, so it says nothing about your own traffic |
| `503` with `errorType: "DATABASE_BUSY"` and `Retry-After: 1` | The instance had no database connection to give inside its bound | Safe to repeat unchanged after the wait — once, from one place, not from every open tab |
| `422` with `errorType: "STATEMENT_BUDGET_EXCEEDED"` and **no** `Retry-After` | The request itself is too expensive for this instance | **Never retry it unchanged** — it is refused identically. Send a smaller request |

**`Retry-After: 1` does not mean the same thing on every row**, and this is the difference most likely to be coded wrongly: on the occupancy refusals and on the `503` it means *the obstacle is a request that ends shortly*, while on a per-minute budget it means *the window has this many seconds left* and can say up to 60. Obey the header everywhere, and never carry a backoff derived from one over to another.

**A write that conflicts with a related record is a `409`, not a crash.** Where the database itself refuses a write because a reference cannot be satisfied — in either direction: something still points at a row being removed, or a row being written points at something that is no longer there — the refusal reaches you as `409` with `errorType: "REFERENCE_CONSTRAINT_VIOLATION"` and `detail: "This change conflicts with a related record, so it was not applied. Something else may have changed at the same time — reload and try again in a moment."` Nothing is half-applied. **This is a property of the API, not of a set of endpoints** — it is what makes "no reference is ever left stranded" true of every path, including one added later that forgets to check, which is the entire reason it exists. **You should not normally meet it.** Where an operation can foresee the conflict it checks first and answers its own, more specific `409` naming what is in the way and what to do about it — for a catalog delete, the remap-or-archive refusal under [System administration](#system-administration) — and that pre-check is the refusal to code against. This one is the backstop under it, and it **prescribes no remedy specific to either direction**: it cannot tell which of the two it caught, and their remedies are opposite, so it offers only the recovery that holds both ways — re-read, then retry. Whether that recovery helps follows from the direction you actually hit. Where the cause was somebody else's commit — something you referenced deleted between the read that resolved it and the write that landed, which is the likeliest way to meet this at all — a retry genuinely can succeed, the exact opposite of the time-budget `422` above; where something still points at what you are removing, it is still there and a retry is refused identically. Re-read first and send what you find rather than replaying the identical body. **"In a moment" is literal:** a reload may still be served a briefly cached copy of the project's catalog, so an entry that is already gone can keep being offered for a little while. There is deliberately no `Retry-After` — that header stays with the lock-contention `409` above, where the wait has a known bound — and nothing here is a schedule to key on. If this paragraph reads as though it was written three times, it was: each round found a claim that was true of one direction and stated for both, which is the mistake this shape of refusal invites.

**A value longer than the column that had to hold it is a `400`, not a crash.** Where the database refuses a write because a string overflowed its column, the refusal reaches you as `400` with `errorType: "VALUE_TOO_LONG"` and `detail: "Some of the text you submitted is too long."` Nothing is half-applied. **This is a property of the API, not of a set of endpoints** — it is applied where the database reports the overflow rather than at a chosen list of handlers, so any write can in principle answer it and there is no list of paths to code against. **You should not normally meet it.** A field that is too long is refused at the door by a field-anchored `400` that [names it](#validation-failures-400), or by a service check that names the limit, and those are the refusals to code against. This one is the backstop under them and it **names no field**: by the time it is raised the request body is gone and the database has reported a width rather than a column, so it says what it knows instead of promising a field it cannot supply. That is also how you tell it from a validation failure — it carries an `errorType` and **no `errors` map**. There is no `Retry-After` and an identical retry is refused identically; what clears it is shortening the longest free text in the request. Meeting it at all means some path is missing a bound, so the server records it as a fault even though it answers as a client error — worth reporting.
...
**`errorType` says which failure a body is**, wherever one status covers more than one. It is a plain string extension, stable and safe to branch on where the wording of `detail` is not: an [invalid HQL query](#search-hql) carries `PARSE_ERROR` or `SEMANTIC_ERROR`, a member removal refused over project administrators carries `STRANDED_PROJECTS`, `ADOPTION_BLOCKED` or `ADOPTION_ROLE_UNREADABLE`, a change that would leave a project administered by nobody at all carries `STRANDED_BY_INHERITANCE`, and any request stopped for outrunning the server’s time budget carries `STATEMENT_BUDGET_EXCEEDED`, a write the database refuses because a reference could not be satisfied carries `REFERENCE_CONSTRAINT_VIOLATION`, and one it refuses because a string was wider than its column carries `VALUE_TOO_LONG`, a request refused because the instance could not obtain a database connection in time carries `DATABASE_BUSY`, and an [attachment upload refused because the workspace has no storage left](#attachments) carries `STORAGE_QUOTA_EXCEEDED` — the one on that list which **must not be retried**, since a quota does not refill. Two rules for consuming it: treat a value you do **not** recognise as "no recovery I know about" rather than guessing at one, and do **not** read its *absence* as an unknown value — a response that needs no discriminator simply has none (the lock-contention `409` above is exactly that case, and is identified by `Retry-After` instead). It is also **not a `409`-only member**: the [time-budget `422`](#errors) above carries it, so does the [`503` raised when the instance has no connection to give](#errors), one `403` carries it too — [`REACTIVATED_DEFAULT_ABOVE_CEILING`](#reactivated_default_above_ceiling--a-403-that-names-projects), when restoring open project access would bring back a per-project default the caller may not set — and the very same endpoints answer the plain [grant-ceiling](#the-grant-ceiling) `403` with none at all, so the absence rule matters there exactly as much as it does for lock contention. Being a `403` it stands outside the `Retry-After`-first rule, which sorts `409`s only. A `429` carries one too, on the [expensive-read surface](#rate-limits), and there the absence rule does real work: a refusal about how many requests are *running at once* names itself, while the per-minute budget carries none — so a `429` without an `errorType` is the budget rather than a cause you do not recognise, and the two read `Retry-After` differently.
...
| Status | Meaning |
|---|---|
| `400` | Malformed request or failed validation — **plus** a write the database refused because a value was [wider than its column](#errors) (`errorType: "VALUE_TOO_LONG"`), which names no field and carries no `errors` map |
...
| `409` | Conflict: stale `version`, duplicate name/key, resource in use, or a state invariant (the last owner, the last project administrator, a [built-in role](#built-in-roles-are-read-only)) — **plus** two cases where the state itself may have moved: a lost row-lock race, which carries `Retry-After` and wants the identical request again, and a [reference conflict](#errors) (`errorType: "REFERENCE_CONSTRAINT_VIOLATION"`), which carries no header and wants a fresh read before the retry. Several of these carry an `errorType` discriminator; see [the role `409`s](#the-409-codes-on-role-writes) for the full list and the order to check them in |
| `413` | Attachment exceeds the per-file size limit (default 20 MB) or the servlet upload ceiling (default 25 MB) — or a [browser CSP report](#browser-violation-reports-csp) whose declared body length is over 16 KB |
| `415` | Attachment file extension is not in the allow-list — or a [browser CSP report](#browser-violation-reports-csp) sent as anything but the three report media types |
| `422` | Semantically invalid reference (unknown status/type/assignee, a user who [may not be assigned](#assignability--a-422-not-a-403) in this project, workflow-forbidden transition, an [unusable role reference](#unknown-role--a-422), an [unknown or wrong-scope permission](#scope-is-immutable-and-permissions-must-match-it)) or a request that names a role with [neither or both](#naming-a-role-roleid-and-the-deprecated-role-key) of `roleId` / `role`. **Also**, on `register` and `reset-password` only, a `password` / `newPassword` that cannot be stored as sent — either [a value this project published](#a-password-this-project-published--a-422) or one [longer than 72 UTF-8 bytes](#a-password-longer-than-72-utf-8-bytes--a-422) — and, on the [custom-field upsert](#system-administration), a `config` past its document, option-count or option-text ceiling, or one carrying a select option whose `color` is not a [hex color](#conventions). **Plus** the app-wide [time-budget refusal](#errors) (`errorType: "STATEMENT_BUDGET_EXCEEDED"`), which any request can answer and which carries no `Retry-After` — do not retry it |
| `429` | Rate limited — wait the number of seconds in the `Retry-After` header. **Where the refusal is spent differs by surface**: every budget an interceptor spends — and the occupancy bound beside them — is spent before the resource is resolved, so its `429` can precede a `404`; the [invitation ceilings](#invitations-are-throttled) are spent after it, so a workspace you cannot see answers `404` even when you are over one. Read that as the category rather than as today's list of surfaces: it is a property of where the refusal is spent, and it holds for a surface that grows a budget tomorrow. See [Rate limits](#rate-limits) |
| `503` | The instance could not obtain a database connection inside its bound (`errorType: "DATABASE_BUSY"`), with `Retry-After: 1`. **Any** endpoint can answer it, unauthenticated ones included, and no list of paths returns it — see [Errors](#errors). The transaction that failed applied nothing; whether the *request* is safe to repeat is a property of the operation, not of this status. Retry after the wait once, from one place, rather than from every open tab at the same moment |

### Rate limits

The sensitive auth endpoints (`login`, `register`, `verify-email`, `resend-verification`, `forgot-password`, `reset-password`) share a **per-IP budget** (default 15 requests per minute). Additionally, repeated failed logins for one account trigger an **exponential backoff** (defaults: starts at 30 s after 5 consecutive failures, doubles per failure, capped at 15 min); a successful login resets the counter. Both mechanisms respond with `429` and a `Retry-After` header (seconds). **One refusal is refunded, up to a ceiling**: when a request on these endpoints is refused with `503` [`DATABASE_BUSY`](#errors), the per-IP token it spent is given back — a refusal carrying `Retry-After: 1` would otherwise drain that budget in as many seconds as it has tokens and lock a caller out of the door they need most in the middle of an incident. **The refund is itself bounded, and this is the part to code against: one window gives back at most one budget's worth of tokens**, so an address can spend at most **twice** the budget inside one minute and the door still has a ceiling. The token did not buy nothing — the request held a worker for the whole connection-acquisition wait — and an unbounded refund would leave one address with no per-IP bound at all for as long as the pool stayed starved. Concretely, at the default budget of 15 and one request per second: the first 15 refusals are refunded, the next 15 are charged, and the 31st request in that minute is an ordinary `429` — roughly half a minute into an incident. Obey its `Retry-After` as you would any other; the next window starts with a full budget and a full refund allowance. Nothing else is refunded: a rejected password, a malformed body and a `500` all spend, because only the `503` means the instance never got as far as the work. Operators tune or disable this via the `RATE_LIMIT_*` variables below. Counters are in-memory (per app node).
**Reset and verification mail carry a per-address ceiling, and on two endpoints it is the one throttle in this API that answers nothing.** `POST /auth/forgot-password`, `POST /auth/resend-verification` and `POST /auth/register` will each send at most one message per address per minute, and at most 5 per window — **15 minutes for reset mail, an hour for verification, an hour for registration** — counted **per address and per kind of mail** across everybody who asks, not per caller and not per IP, because the account is what is under attack and an IP is what an attacker has the most of. **All three keep separate budgets**, including register and resend-verification, which mail the same message: a shared bucket would be fillable through whichever endpoint is cheapest to spend, and resend-verification spends a slot at an address with no account while sending nothing at all. The ceiling counts the destination **inbox**, not the spelling: `a+1@gmail.com`, `a+2@gmail.com` and `a.1@googlemail.com` are one bucket, so registering several accounts at one address spends one allowance.

On `forgot-password` and `resend-verification`, past a ceiling the request still answers `200` with the same sentence as always and simply sends nothing. **Those two ceilings deliberately have no `429` of their own and no `Retry-After`**: those endpoints answer identically for a registered and an unregistered address, which is what stops them being an account-enumeration oracle, and a refusal visible to the caller would publish — to anybody, for free — that *somebody* asked for a reset at that address a minute ago. (Both endpoints can still answer `429`, but only from the per-IP budget above — a refusal about the caller's request rate that says nothing about any address.) The ceiling is spent **before** the account is looked up and counted whether or not the account exists, so being throttled says nothing about who is registered. Two consequences worth coding against: a client cannot distinguish "sent" from "suppressed" and must not try, and a **resend button should be debounced client-side** rather than relying on the server to say no, because the server will not.

`POST /auth/register` spends a per-inbox budget **of its own**, and it **does** answer `429` with `Retry-After`. The difference in refusal shape is a property of the endpoint rather than of the mail: register already discloses whether an address is taken, through its own `409`, so a refusal there adds one folded-key bit to a caller who has just been told about the exact spelling anyway. It refuses the **registration**, not the mail, and above the account row: nothing is created, and the address stays free. (Dropping the mail instead would leave an account nobody — its owner included — could ever activate.) The budget is separate from `resend-verification`'s even though the two send the same message, because a shared bucket is fillable through whichever endpoint is cheapest to spend, and resend-verification spends a slot at an address with no account while sending nothing at all — which made denying a stranger's signup free, silent and refillable. The `429` on register therefore has two independent causes, the per-IP budget and this one, and both carry `Retry-After` — but they are not the same refusal, and which one a caller met changes what you can honestly tell them. **The per-IP one is about the caller's own traffic**: its `detail` is the generic “too many requests” sentence and its `Retry-After` is only the remainder of the current minute, so slowing down clears it. **The per-inbox one is not about the caller at all** — its `detail` talks about verification mail to this address rather than about a request rate, and that budget is spent by everybody who asks, so retrying from another network, another client, or another spelling of the same inbox changes nothing; only waiting does. That wait is either the per-inbox cooldown (one minute by default) or, at the per-window cap, the moment the oldest registration for that inbox ages out **rounded up to the next 15-minute boundary** — an absolute deadline rather than a countdown, so it can exceed an hour and `now + Retry-After` names the same instant however often you ask. In code, obey `Retry-After` in both cases and do not branch on the wording of `detail`: the remedy is the same wait either way, and only the advice you show a human differs.

These ceilings live in the database, so they are exact instance-wide and unaffected by a restart or by which node served the request.

Several non-auth surfaces have throttles of their own. [`POST …/issues/{number}/rank`](#sprints--backlog) allows at most one whole-project rank *rebalance* per project per 60 s and answers `429` + `Retry-After` for a second one inside that window; it is a retryable throttle, not a fault — nothing was moved. That cooldown is a fixed internal safety valve with no environment variable **and no off switch**, so this endpoint can still answer `429` on an instance where every limiter that *has* a switch is off — `RATE_LIMIT_ENABLED` reaches exactly the bounds for which it is the only switch, and a bound that carries one of its own (the [storage quota](#workspace-storage), the expensive-read occupancy bound) or carries none at all (this cooldown) is outside it. Its counter is in-memory per app node too. Every [report](#reports) — the six under `…/reports/**` plus [`POST …/search/insights`](#insights--break-down-the-query-in-the-search-box), which is bound to the same limiter explicitly because it does not sit under that path — shares **one per-principal budget of 60 requests per minute** (`REPORTS_REQUESTS_PER_MINUTE` below, in-memory per app node, switched off wholesale by `RATE_LIMIT_ENABLED=false`). And the whole HQL surface `…/search/**` — [`POST …/search`](#search-hql), `…/search/schema`, `…/search/suggest` **and** the insights panel — shares a **second, separate** per-principal budget of **120 requests per minute** (in-memory per app node, and covered by the same `RATE_LIMIT_ENABLED` master switch). [Saved filters](#saved-filters) (`…/filters/**`) are on that second budget too: validating a filter's HQL builds the same resolution context `…/search/schema` pays for, so an invalid-body loop there is the same cost wearing different clothes. The two are deliberately not one pot: somebody typing in a search box legitimately fires several requests a minute and must not be starved to protect charts.

**The planning reads have a pot of their own, and it is sized for a gesture rather than for a chart.** The whole view [`GET …/projects/{pId}/backlog`](#the-planning-view) and [every section of it](#refreshing-one-section) under `…/backlog/sections/…` share **one per-principal budget of 240 requests per minute** (`PLANNING_REQUESTS_PER_MINUTE` below, in-memory per app node, switched off wholesale by `RATE_LIMIT_ENABLED=false`), bound to the whole `…/backlog/**` path rather than to a list of endpoints, so a planning read added later is inside it the moment it exists. It is deliberately not the reports pot: a card dragged across sections costs two section refreshes, so an ordinary grooming session would reach 60/min inside three minutes, and a `429` in the middle of a drag reads as a product defect where a `429` on a chart reads as a delay. Past it the answer is `429` + `Retry-After` — never a narrowed section, a smaller `sectionCap` or a `truncated` that was not earned.

**Asking too often is not the only way to be refused on that surface.** Those same paths — every [report](#reports), the whole of `…/search/**` and `…/filters/**`, the [storage breakdown](#workspace-storage) and [every planning read](#the-planning-view) under `…/backlog/**` — are additionally bounded by **how many of their requests may be running at once**: one bound on the caller's own in-flight requests, and one on the whole instance's, so that the rest of the API always keeps a share of the database connections however busy this surface gets. It is not a rate and it does not refill: what clears it is something finishing. Nothing is computed when one is refused, so the identical request is safe to repeat once there is room, and the refusal is a `429` like the budgets above. What puts an endpoint on that list is the work it does — holding one database connection across many statements — and not the path it is mounted under, so the list grows whenever such an endpoint is added.

**One thing this surface asks of a client is not a status code at all: never answer a refusal by asking for something bigger.** On the planning surface that is concrete — a refused [section refresh](#refreshing-one-section) must not be retried as the whole view, which assembles every open section in one transaction and costs several times what was just refused. The same instinct is wrong everywhere here: a refusal is not an invitation to widen the request.

**A `429` on this surface names its own cause in `errorType`; check it rather than assuming which one you got.** A per-minute budget carries no `errorType` at all, so an absent one means the budget rather than a cause you do not recognise. The occupancy causes:

- **`TOO_MANY_IN_FLIGHT`** — *your own* requests are occupying your share of the surface. Let one finish: close the extra tab, stop the loop, stop firing the same read in parallel. A client may retry this one **once** after the hinted delay.
- **`EXPENSIVE_SURFACE_BUSY`** — the *instance* is running as many expensive requests at once as it can. You may hold none of them, so there is nothing about your own concurrency to change and reducing it need not help. Retry shortly, but **not automatically and not immediately**: retrying a saturated shared resource is what keeps it saturated. Offer a person a manual retry instead.

**`Retry-After: 1` on these two does not mean what `Retry-After` means on a budget, and this is the difference most likely to be coded wrongly.** Here it means *the obstacle is a request that ends shortly* — another in-flight request, typically finishing in tens of milliseconds. On a per-minute budget it means *the window has this many seconds left*, up to 60, and waiting it out is the whole remedy. Obey the header in both cases, but never carry a backoff derived from one over to the other, and never read the small number as permission to retry in a loop. Both occupancy refusals are decided before the workspace or project is resolved, exactly as the budgets are, so they carry no tenancy information either and one of them can likewise precede a `404`. Neither body ever carries a count, a principal or a tenant: `EXPENSIVE_SURFACE_BUSY` says the instance is busy and nothing whatever about who made it busy.

**On a self-hosted instance the numbers are yours**: the four `EXPENSIVE_READ_*` settings in [Operator settings](#operator-settings-that-affect-the-api), where the relation between them and `DB_POOL_MAX_SIZE` is spelled out. That relation is checked at startup rather than trusted to prose — a configuration you *typed* that breaks it does not quietly do the wrong thing, it stops the boot naming both properties, while the shipped configuration derives the share from your pool and therefore cannot break it. Two things worth knowing before you reach for a switch: this bound is **not** under `RATE_LIMIT_ENABLED` (it has one of its own, `EXPENSIVE_READ_LIMIT_ENABLED`), so turning rate limiting off leaves it running; and turning it off leaves nothing at all between one caller and every connection on the replica.

**Writing costs a budget too, and the write surface had none until recently.** Every **mutating** request under `…/projects/*/issues/**` — issue create/update/delete, comment create/update/delete, attachment upload/delete and [backlog rank](#ranking-an-issue) — spends a **per-principal budget of its own** — **180 requests per minute** (`WRITE_REQUESTS_PER_MINUTE` below, in-memory per app node, switched off wholesale by `RATE_LIMIT_ENABLED=false`). It is method-conditioned, so `GET`s on the same paths cost nothing, and it covers `PATCH` and `DELETE` as well as `POST`: a budget that stopped only creation would simply move a caller to the update, and an issue update is not the cheaper request — it writes history rows, bumps the version, and fans out notifications and [SSE](#real-time-events-sse). Uploading an attachment additionally spends a per-principal **byte** budget of **250 MB per minute** (`WRITE_UPLOAD_BYTES_PER_MINUTE`), summed over the **parsed** sizes of your own files, because a request count does not bound bytes. Both answer `429` + `Retry-After`, both are spent before the workspace is resolved, and both are keyed on the caller — so neither refusal says anything about a workspace, and neither is affected by what anybody else is doing. The rank endpoint can therefore answer `429` for either reason, and `detail` says which.

**One refusal in this area is deliberately not a throttle at all.** A workspace has a ceiling on the [attachment storage](#workspace-storage) it may hold, and an upload that would cross it answers **`409`** with `errorType: "STORAGE_QUOTA_EXCEEDED"` and **no `Retry-After`** — because waiting frees no bytes and an identical retry fails identically for ever. Check `Retry-After` before deciding to retry any refusal here; see [Attachments](#attachments) for the full ordering.
...
**Inviting somebody is throttled too, and its `429` behaves differently from the budgets above.** [`POST …/workspaces/{id}/invites`](#invitations-are-throttled) carries three ceilings — a cooldown before the same sender may invite the same address again (60 minutes by default, across every workspace on the instance), a cap on how many invitations one address may receive per day from the whole instance (5), and a per-sender volume budget (20 an hour and 100 a day) — and answers `429` + `Retry-After` past any of them, with nothing in the status to say which. A refusal sends no mail and writes no invitation, so the identical request is safe to repeat after the wait. Unlike the per-principal budgets above, these are spent **after** the workspace is resolved, so this `429` never precedes a `404`: see [Invitations are throttled](#invitations-are-throttled). The five settings behind them are the `INVITE_*` variables below, and `RATE_LIMIT_ENABLED=false` switches all three off along with every other limiter **that has an off switch** — the rank-rebalance cooldown above is the one that has none.

**One bound here is keyed on neither a principal nor a workspace, and it is deliberately nobody else's pot.** The [browser CSP report sink](#browser-violation-reports-csp) carries a volume budget of its own — **60 reports a minute from one client address, 600 a minute across the instance**, counted in memory per app node — and answers `429` + `Retry-After` past either, with no body. It is emphatically **not** the per-IP auth budget above, and the distinction is the point rather than an implementation note: that budget is shared with `login`, `register` and the password-recovery doors, one address is routinely shared by many people, and the caller filling this bound is a browser behaving correctly on a page this instance served. A report flood able to spend a person's login allowance would be a denial of service the feature invented for itself, so this door pays out of its own pot and a refusal here costs nobody the door they need most. It is also the one bound in this section whose refusal no client of this API will meet: its sender is a browser, which reads the status and drops the report.

Everything these per-principal interceptors refuse — the budgets and the occupancy bound alike — is refused **before** the workspace and project are resolved, which is where a `429` precedes a `404` in this API; the [invitation ceilings](#invitations-are-throttled) are the deliberate counter-case, spent after. Being keyed on the principal, the budgets bound **one user, not one workspace**: aggregate load still scales with member count (see [Reports](#reports)). Beyond the bounds named in this section and the auth endpoints above, nothing else in this API is throttled. Note that a bound is not a path: saved filters are throttled without sitting under `…/search/**`, because what earns one is the work a handler does, not where it is mounted.
...
| Method | Path | Auth | Description |
|---|---|---|---|
| `POST` | `/auth/register` | — | Create an account; sends a verification email. `201`. **`403` when public signup is disabled** (the DC default — accounts are created by an admin instead, see [System administration](#system-administration)). `400` when the address, once lower-cased for storage, would not fit `users.email` — [see above](#validation-failures-400); distinct from the ordinary 255-character field bound. `409` when the address is already registered (addresses are matched case-insensitively — see below). `422` when `password` cannot be stored as sent — [a value this project published](#a-password-this-project-published--a-422), or one [longer than 72 UTF-8 bytes](#a-password-longer-than-72-utf-8-bytes--a-422). `429` from the [per-address ceiling](#rate-limits) on verification mail as well as from the per-IP budget — it counts the destination **inbox**, so several accounts at one address share one allowance, and nothing is created when it fires |
| `GET` | `/auth/verify-email?token=` | — | The address the verification email links to. Verifies nothing itself — `302` to the in-app screen, which calls the `POST` below |
| `POST` | `/auth/verify-email` | — | Exchange the emailed one-time token for a session |
| `POST` | `/auth/resend-verification` | — | Re-send the verification email (always `200`). [Per-address ceiling](#rate-limits) of its own, separate from `POST /auth/register`'s: past it the answer is unchanged and no mail is sent — that ceiling never answers `429` |
...
| `POST` | `/auth/forgot-password` | — | Send a reset link (always `200` — no account enumeration). [Per-address ceiling](#rate-limits): past it the answer is unchanged and no mail is sent — that ceiling never answers `429` |
| `POST` | `/auth/reset-password` | — | Set a password with a one-time token; revokes all sessions **and [every other outstanding reset link](#redeeming-a-reset-link-retires-every-other-outstanding-one) for that account**. Backs both the forgot-password email and admin-generated account setup links. `422` when `newPassword` cannot be stored as sent — [a value this project published](#a-password-this-project-published--a-422), or one [longer than 72 UTF-8 bytes](#a-password-longer-than-72-utf-8-bytes--a-422); the token stays usable either way |
| `GET` | `/auth/me` | ✔ | The current user (`id`, `email`, `displayName`, `avatarUrl`, `systemRole`, `needsOnboarding`) |

Every body in this table that carries an email address caps it at 255 characters — see [validation failures](#validation-failures-400). Every one-time token is capped at **64 characters**, in a body or as a query parameter (they are 43 characters as generated, so the cap is generous and finite; the column behind a token stores a fixed-width hash and could never be the thing that refuses). `password` is capped at **1024** on `login` and deliberately carries no minimum there — a length rule on a door that only *verifies* a password would answer `400` where the endpoint must answer `401`, and would tell an attacker which submissions were too short to be anyone's. The doors that **write** a password are bounded far more tightly, and in two units: see [below](#a-password-longer-than-72-utf-8-bytes--a-422). Those addresses are **lower-cased before anything is looked up**, and account uniqueness is enforced on that lower-cased form: `Ivan@example.com` and `ivan@example.com` cannot both be accounts, so registering the second is a `409`. Casing in the address you submit therefore never changes which account you reach.
...
### Redeeming a reset link retires every other outstanding one

**Redeeming any password-reset link retires every other outstanding one for that account, whichever door minted it.** A successful `POST /auth/reset-password` performs two revocations in one act: it ends every active session, and it spends the account's whole outstanding set of reset tokens rather than only the one presented. A sibling link used afterwards answers the same `400` as a token that is unknown, expired or already spent.

Each link was always single-use; the *set* of them was not. Several can be live at once — the [per-address ceiling](#rate-limits) allows a handful of sends inside its window, and a "send another link" control makes accumulating them ordinary — so an unspent link copied out of a mailbox used to outlive the reset its owner performed in order to defeat it. It no longer does.

**The rule is about the capability, not about which endpoint issued the link.** What a completed reset withdraws is the account's outstanding right to set a password without knowing the current one, so every link carrying that right ends together — including the setup link an administrator hands over ([System administration](#system-administration)), which is the same capability with a longer life. Sparing it would have spared the longer-lived half of the same primitive; an administrator who still needs one issues a fresh link from the console.

**A door that takes no authentication may never retire a link.** `POST /auth/forgot-password` leaves links already sent working, and that is a rule rather than a convenience: anyone can name any address there, so if asking for a link voided the earlier ones, anybody who can type a victim's address could keep killing the link that victim is waiting on. Retirement is earned by proving possession of a token, and only there.

**The prohibition is a property of authentication, not of minting**, so do not read it as "issuing never retires". `POST /admin/users/{id}/setup-link` mints too and also leaves earlier links working ([System administration](#system-administration)) — but it is authenticated, and nothing above compels that behaviour; it is a question deliberately left open rather than answered here. Take an authenticated door's behaviour from its own entry.

### A password this project published — a `422`

Two endpoints refuse one specific password value: `POST /auth/register` (`password`) and `POST /auth/reset-password` (`newPassword`).

```json
{ "type": "about:blank", "title": "Unprocessable Content", "status": 422,
  "detail": "That password is published in Hamstrack's own configuration template, so it is known to everyone who can read the project's source. Choose a different one." }
```

The refused value is one Hamstrack has **itself published** as production configuration. Today there is exactly one: the literal `SEED_ADMIN_PASSWORD`, which shipped in `.env.prod.example` as the seeded administrator's password and is therefore readable by anyone who has the project's repository URL. An instance running it is not weakly protected, it is publicly administrable — and because a correct password is not a failed login attempt, the [per-account backoff](#rate-limits) never engages against somebody using it. The same predicate refuses it at startup, so what the running application accepts and what the next boot tolerates cannot come to disagree.

**This is not a password-strength check, and it is not a denylist.** It holds only values *we* printed, under their own variable's name. A weak password this project never published is accepted exactly as before, subject only to the ordinary length rules in the next section and to ordinary [validation](#validation-failures-400). Do not surface it as "your password is too weak" — the accurate sentence is the one in `detail`: that particular value is public, so choose another.

**On `reset-password` the link survives the refusal.** The check runs *before* the one-time reset row is consumed, so a caller who meets this `422` simply retries on the same link with a different password. That is what separates it from the `400` on the same endpoint, which means the link itself is invalid, expired or already spent. On `register` nothing is created either — no account and no verification email — and the check precedes the email lookup, so a request that is *also* a duplicate registration answers this `422` rather than the `409`.

`400` would be the wrong status: the request is well-formed and every syntactic constraint on the field is satisfied, and it is the *value* that cannot be honoured — the same shape as [`"Unknown role"`](#unknown-role--a-422). Branch on the status, not on the wording of `detail`.

### A password longer than 72 UTF-8 bytes — a `422`

The two endpoints that **write** a password — `POST /auth/register` (`password`) and `POST /auth/reset-password` (`newPassword`) — bound it in two units, and the two bounds refuse with two different statuses:

- **more than 72 characters → `400`**, field-anchored like any other [validation failure](#validation-failures-400), with `password` / `newPassword` in the `errors` map;
- **72 characters or fewer but more than 72 UTF-8 bytes → `422`**, with no `errors` map:

```json
{ "type": "about:blank", "title": "Unprocessable Content", "status": 422,
  "detail": "That password is 96 bytes long and the maximum is 72 bytes. Characters outside the basic Latin set count for more than one: accented letters, Greek and Cyrillic cost 2 bytes each, most other scripts 3, and emoji 4 — so a passphrase can be well under 72 characters and still be over the limit. Shorten it, or use fewer non-Latin characters." }
```

**Do not tell a user the limit is "72 characters".** It is 72 characters *and* 72 bytes, and outside the basic Latin set the byte limit is the one that binds first: accented Latin, Greek and Cyrillic cost 2 bytes per character, most other scripts 3, emoji 4. So a Cyrillic passphrase is refused at about 37 characters, a CJK one at about 25 and an all-emoji one at 19 — each comfortably inside any character counter a form might show. If you render a hint, render it in the unit the reader can act on, which is what `detail` does: the byte count of what was sent, the limit, and the arithmetic between them. A client-side `maxlength` is a courtesy and not the guard — an `input` counts characters and cannot see this.

The bound is a property of the password hash this application writes rather than a policy anyone configured, so it is the same on every instance and in both deployment models and it does not move with a setting. **Nothing is locked out by it**: no account can hold a longer password, because the same hashing refused to create one.

**On `reset-password` the link survives this refusal**, exactly as it survives the [published-password `422`](#a-password-this-project-published--a-422) — the check runs before the one-time reset row is consumed, so retry on the same link with a shorter password. On `register` nothing is created and no verification email is sent.

`POST /auth/login` has no rule of this kind and needs none: it *verifies* a password rather than storing one, and verification neither fails nor costs more for a long submission. Its `password` is capped at 1024 purely so that the field is finite.
...
| `POST` | `/workspaces/{id}/invites` | `workspace.member.manage` | Email an invite (`{"email", "roleId"}`, or the deprecated `role` — [exactly one](#naming-a-role-roleid-and-the-deprecated-role-key); subject to the grant ceiling, never `OWNER`). The address is capped at [255 characters](#validation-failures-400), separately refused if folding it for storage would push it past that same bound, and its [local part must be ASCII](#an-invited-address-must-have-an-ascii-local-part-400) — three separate `400`s. `409` if they are already a member, or if [an invitation to that address is already standing](#one-invitation-per-address-409) — two refusals told apart by `errorType`, not by the status. [Throttled](#invitations-are-throttled): `429` + `Retry-After` past an invitation ceiling, and a refusal sends no mail and writes no invite. `201` |
...
| `DELETE` | `/workspaces/{id}/invites/{inviteId}` | `workspace.member.manage` | [Withdraw an invitation](#pending-invitations-seeing-them-and-withdrawing-one), and the only way to [free the address for a fresh one](#one-invitation-per-address-409). `204`, including for an expired one; `404` once it is gone (treat as success); `409` if it was accepted meanwhile |
...
| `GET` | `/workspaces/{id}/storage` | member | [How much attachment storage this workspace uses](#workspace-storage), and how much it may use |
| `GET` | `/workspaces/{id}/storage/projects` | `workspace.edit` | [The same total broken down by project](#workspace-storage), largest first. On the [reports budget](#rate-limits) |
| `POST` | `/workspaces/accept-invite?token=…` | ✔ | Accept an invite; must be signed in with the invited email |

`slug` is derived server-side from the name, never accepted from a client and never regenerated on rename. It is **clipped to 100 characters**, so a long workspace name yields a short slug instead of a failure — the full name is always beside it in `name`.
...
- deletes the **unaccepted invite** for their address in this workspace, if one is left over — [at most one can be](#one-invitation-per-address-409), and it stays hidden while the person is a member, so without this, removing them would make it *reappear* on their "join a team" screen, and one click would put them straight back. It also frees the address, so a removed person can be invited again immediately. Accepted invites are kept as the record of how they originally joined;
...
### An invited address must have an ASCII local part (`400`)

**`POST /workspaces/{id}/invites` refuses an address whose local part — everything before the last `@` — contains a non-ASCII character.** It is an ordinary [validation `400`](#validation-failures-400) keyed `email`, the same shape as the 255-character cap, and it is a *delivery* rule rather than a style rule.

**Why it exists.** The invitation is stored under, and **mailed to**, the address case-folded (lower-cased), and the [recipient ceilings](#invitations-are-throttled) derive their key from that same value — so the address a ceiling binds is the address the message is written to. That fold is also what makes a non-ASCII local part dangerous: lower-casing collapses U+212A KELVIN SIGN onto a plain `k`, so an address spelled with one would deliver a workspace name and a **live join token** to a *different real person* — the one who owns the ASCII spelling, whose own account address therefore matches the folded invitation exactly, which is why they could go on to accept it. Refusing at the boundary takes away nothing that was ever deliverable: there is no SMTPUTF8 path here, so such an address could not have been sent as typed, and a silent redirect to a stranger has no upside to weigh against it.

**Non-ASCII is refused *before* the `@` and accepted *after* it, and the two halves differ because the transformations do.** Folding a local part changes **which mailbox** receives the message. The only thing that ever happens to an internationalised domain is punycode, and only inside the throttle key: the `xn--` form and the Unicode spelling are one domain to a resolver, so that is a normalisation onto the same host rather than a fold onto somebody else's mailbox — and the message itself is addressed to the domain you sent. So `you@пример.рф` is accepted while a local part carrying a Kelvin sign is not, and that is not an inconsistency.

**The check is anchored on the last `@`.** A quoted local part that contains an `@` of its own — `"a@b"@example.com`, which the email format rule accepts — still passes, because everything before the final `@` is ASCII.

Two things to code against. This `400` is validation, so it is answered before anything else the endpoint does: it spends none of your [invitation allowance](#invitations-are-throttled), and it does not depend on rate limiting being enabled. And per [Validation failures](#validation-failures-400) only **one message per field** is reported, so an address that breaks both the format rule and this one comes back with one of the two and not both — branch on the `errors` key (`email`), never on the wording.

### One invitation per address (`409`)

**A workspace holds at most one unaccepted invitation per address.** While one is standing, inviting that address again is refused with `409` and `errorType: "DUPLICATE_INVITE"`: the standing invitation is not replaced, not re-sent and not extended.

That is a deliberate refusal to do something silently. Withdrawing a standing grant has [an endpoint of its own](#pending-invitations-seeing-them-and-withdrawing-one), with its own audit trail and its own confirmation, and `invite` does not perform it under another verb — nor does it quietly restart the seven-day life of a link. **To change the role or send a fresh link, withdraw the standing invitation and invite again:** two calls, against a row listed on the same screen as the form.

**An expired invitation still blocks, and only a withdrawal clears it.** Nothing in this product sweeps invitations, and the rule cannot exclude expired rows even in principle — a database index can only enforce a condition that does not change with the clock, and "not yet expired" does. So a lapsed invitation keeps the address reserved until somebody withdraws it, which `DELETE /workspaces/{id}/invites/{inviteId}` does on expired rows too. That is also why [the list shows expired rows](#pending-invitations-seeing-them-and-withdrawing-one) carrying `status: "EXPIRED"`: **the rows that can block an invitation and the rows an administrator can see are one set.** Labels, components, versions and sprints already behave this way — a dead row keeps its slot until somebody clears it, and the refusal's job is to say who clears it and where.

The shape, not the sentence, is what to code against:

```json
{
  "status": 409,
  "errorType": "DUPLICATE_INVITE",
  "detail": "There is already an invitation to bob@x.com waiting in this workspace. …"
}
```

There are **two wordings and one `errorType`**, chosen by whether the blocking invitation has lapsed — the second says it is still *on file* rather than still in flight. Both name the address you submitted and the control that frees it, and both need only the permission you have just proved. Match on `errorType`, render `detail` verbatim, and parse neither.

**Two different `409`s now share this endpoint, and `errorType` is what tells them apart — not the status, and not the wording.** The older one, the invitee is **already a member**, carries **no** `errorType`, and it **wins when both apply**: somebody can be both a member and the addressee of a leftover unaccepted invitation, and "they are already in this workspace" is the more useful answer. Per [Errors](#errors), read an absent `errorType` as "not this refusal" rather than as an unknown one.

**A duplicate is refused above the [ceilings](#invitations-are-throttled), so it spends no allowance.** Retrying one costs neither your hourly nor your daily budget and does not move the recipient's cooldown — exactly as a `400`, a `403` or a `422` from this endpoint costs nothing.

**Uniqueness and the ceilings normalise the address differently, on purpose, and the difference is visible from the client.** Uniqueness is about the **offer**, and an offer is redeemed by an exact match against the account that accepts it — so it compares the address as written, where an extra match would refuse an invitation to a genuinely different person. A ceiling is about the **delivery**, so it counts one destination inbox however the address is spelled, where an extra match merely refuses sooner. Each folds as far as its own harm points. So `victim@x.com` and `victim+a@x.com` are two invitations and one inbox: the second is never refused as a duplicate, and may well be refused as a [`429`](#invitations-are-throttled).

### Invitations are throttled

**`POST /workspaces/{id}/invites` has ceilings of its own, and past one of them it answers `429` with a `Retry-After` header (seconds).** A retryable throttle, not a fault.

**This `429` comes *after* the `404`, which is the opposite of every other throttle in this API.** Every [budget an interceptor spends](#rate-limits) — and the occupancy bound with them — is spent before the workspace is resolved, so a `429` from any of those says nothing about whether the caller can see the resource. Here the ceilings are spent **last**, after every other check the endpoint makes: the workspace is resolved first (an unknown workspace and one you are not a member of are the same `404`), then [`workspace.member.manage`](#permissions) (`403`), then the role reference and the [grant ceiling](#the-grant-ceiling) (`422` / `403`), then "already a member" (`409`), then [a standing invitation to the same address](#one-invitation-per-address-409) (`409`) — and only then the ceilings. **A workspace you are not a member of answers `404` even when you are over every ceiling**, so a `429` here is only ever seen by a member who may invite. The inversion is deliberate: a refusal keyed on a *recipient* spent before the tenancy check would answer, to a non-member, a question about invitation traffic elsewhere on the instance. So the rule that holds for those budgets — that a `429` says nothing about whether you can see the resource — is exactly the rule that does not hold here. One corollary worth relying on: no refusal above the ceilings spends any of your allowance, so being refused for a role or a workspace costs you nothing.

**There is more than one reason for this `429`, and the status does not tell them apart:**

| Ceiling | Keyed on | Default |
|---|---|---|
| A cooldown before the **same sender** may invite the **same address** again — across every workspace on the instance, not only this one | (sender, inbox) | 60 minutes |
| A cap on invitations **one address** may receive per day from the whole instance, whoever sends them | inbox | 5 per day |
| A volume budget on invitations **one account** may send, over two independent windows — spending the hourly allowance repeatedly ends the day, and the daily refusal does not lift when the hour rolls | sender | 20 per hour, 100 per day |

**The two address-keyed ceilings count the destination inbox, never the spelling of the address.** Spellings that a mail provider delivers to one mailbox are one key here, so re-spelling an address buys neither a fresh cooldown nor a fresh daily slot: invite an address, then invite a different spelling that reaches the same inbox, and the second call is refused by a ceiling the first one already spent — a `429` that the address as typed would not predict. Which spellings a provider folds together is a property of the destination and is refined over time, so rely on "one inbox, one slot" rather than on a list of transformations — with one boundary that is not going to move: **look-alike characters are never folded**. What gets folded is a published fact about how a provider delivers mail, whereas whether two different characters look alike to a human is a guess about who somebody is — and two addresses differing by one Cyrillic letter are two people. **Mail is delivered to the address as submitted apart from case** — it is lower-cased first, and the invitation records that lower-cased spelling — but nothing beyond that: the inbox folding described here is a throttle key and never a recipient. That single case fold is the whole reason a [non-ASCII local part is refused](#an-invited-address-must-have-an-ascii-local-part-400).

The `detail` differs per ceiling — it names the address for the cooldown, which is your own past action and discloses nothing, and stays deliberately terse for the per-address cap, whose richer wordings would describe another tenant's activity. **Branch on the status and `Retry-After`, never on the text**: the wording is prose and may change. None of these carries an `errorType`, and per [Errors](#errors) an absent `errorType` is not an unknown one.

**A refused invitation sends no mail and creates no invitation.** The ceilings are spent before either, so nothing happened — repeating the identical request once `Retry-After` seconds have passed is safe, and from the caller's side the endpoint is idempotent across a refusal. Invitations you already sent are untouched: re-send the address that was refused, not the batch.

**`Retry-After` is a real wait, and on one of the three ceilings here it is deliberately coarse.** For the cooldown and the per-sender volume budget it is the exact seconds until your own slot frees — every second of that precision is precision about your own past sends. For the per-address daily cap it is the moment the oldest send in that window ages out **rounded up to the next 15-minute boundary**: an absolute deadline rather than a rounded-off countdown, so `now + Retry-After` names the same instant however often you ask, and what it publishes is which 15-minute bucket the slot frees in rather than the instant itself. That deadline is derived from a send you may not be able to see, which is the whole reason it is coarsened — and the coarsening removes the cheap single-request version of that question rather than putting the instant out of reach, since a caller who simply keeps retrying until the refusal lifts learns it at their own polling resolution, and no shape of this header closes that against a sliding window. On a daily ceiling the wait can be hours; it never under-states the wait, it is never `0`, and apart from the status it is the only machine-readable thing about the refusal.
...
**Withdrawing is also how an address is freed for a fresh invitation**, which is the load-bearing half of this endpoint rather than a side effect. A workspace holds [at most one unaccepted invitation per address](#one-invitation-per-address-409), and an unaccepted one blocks a new one *even after it has expired*, so this is the only way to re-offer access at a different role or with a fresh link — the `DUPLICATE_INVITE` refusal sends you here by name. Two obligations follow, and they are why expired rows are both withdrawable here and listed above: this endpoint must keep working on them, and the list must keep showing them.
...
**Withdrawing is not a way to reset anything.** The [invitation ceilings](#invitations-are-throttled) are keyed on a destination inbox, on a sender, or on both, and are counted independently of the invitation row, so a withdrawal frees none of them: **`invite → withdraw → invite` is refused with `429` exactly as `invite → invite` would be**, and the `Retry-After` is unchanged by the withdrawal. This is the property most likely to be assumed the other way round, and it is visible from the client. The rule behind it, which holds for any revocation this API grows: a revocation frees only a resource whose count it actually reduces — outstanding rows, a uniqueness slot, a stock cap — never one measured over time. Deleting the record of an offer does not delete the record of a delivery. What a withdrawal *does* free is exactly such a count — the [uniqueness slot](#one-invitation-per-address-409) — which is why the retry gets as far as a ceiling at all, instead of being refused one step earlier as a duplicate.

### Workspace storage

Attachments are the only thing a workspace holds by the byte, and a workspace has a ceiling on how many of those bytes it may hold. Two reads publish the figures, with deliberately different gates.

```json
// GET /workspaces/{id}/storage
{
  "quotaEnabled": true,
  "quotaBytes": 107374182400,
  "usedBytes": 8321499136,
  "availableBytes": 99052683264,
...
**Read `quotaEnabled` first and narrow on it.** With the quota switched off there is no ceiling, so `quotaBytes`, `availableBytes` and `percentUsed` are **`null`** — absent numbers rather than a sentinel, because a `-1` is a value a client eventually renders as "-1 GB remaining" and a `0` reads as "full". Everything that describes *usage* rather than the ceiling is always present: usage is counted and reported whether or not anything is being refused, which is what lets an operator see what a quota would have to be before switching one on. Bytes are raw integers everywhere; formatting is the client's business.

`percentUsed` is **not clamped at 100**: lowering the ceiling below current usage is a legitimate operator action, existing files stay readable, and clamping the number would hide the one state that explains why new uploads are being refused. `availableBytes` *is* clamped at 0, because "-2.1 GB remaining" is not something a reader can act on. `asOf` is the usage counter's own timestamp and is `null` for a workspace that has never held an attachment. `maxFileBytes` is the per-file ceiling (`ATTACHMENT_MAX_FILE_SIZE`), carried here so one response answers both "will this file be accepted" questions. `warnAtPercent` (`STORAGE_QUOTA_WARN_PERCENT`) changes no server behaviour at all — nothing is refused, narrowed or slowed at that level; it exists so the first thing a workspace hears about its ceiling is not a refusal.

**The summary needs no permission beyond membership, and is not throttled.** It returns exactly the figures the [quota refusal](#attachments) already hands the same person, so withholding them would only stop a member telling "I am blocked" apart from "the server is broken"; and it is one primary-key read plus two settings — cheaper than any endpoint that *is* behind a budget, and it is the number a client shows beside the upload control, so starving it would hide the ceiling from the person about to meet it. `404` for an unknown workspace or a non-member, as everywhere; never `403`.

The breakdown is different on both counts:

```json
// GET /workspaces/{id}/storage/projects
{
  "asOf": "2026-09-03T10:14:22Z",
  "totalBytes": 8321499136,
  "unattributedBytes": 0,
  "projects": [
    { "projectId": "…", "key": "HD", "name": "Hamstrack", "bytes": 5348024320, "attachmentCount": 902 }
  ]
}
```

It names projects and their volumes — including projects the caller is not a member of — so it is real disclosure and requires **`workspace.edit`**: `403` naming the permission for a proven member without it, `404` (never `403`) for a non-member or an unknown workspace. And it is a grouped aggregate over every attachment row in the workspace — work proportional to what the tenant holds rather than to what the response returns — so it spends the **[reports budget](#rate-limits)** (`REPORTS_REQUESTS_PER_MINUTE`, 60/min by default) and is inside the same bound on how many expensive reads may run at once (`EXPENSIVE_READ_*`). Past either it answers `429` + `Retry-After`, with `errorType` naming which — an absent one is the budget (see [Rate limits](#rate-limits)). The asymmetry with the unbudgeted summary runs the safe way round: a caller refused here who falls back to the summary gets *less* information, never a way around the bound.

`projects` is sorted by `bytes` descending and is empty for a workspace holding no attachments. `unattributedBytes` is `totalBytes` minus what those rows add up to — the drift between what the quota enforces and what the attachment rows say — and it is **published rather than normalised away**, because a non-zero value is the one state this endpoint exists to make visible. It can be negative; a background reconciler corrects the counter on a schedule rather than the response hiding the difference.

Neither endpoint is affected by a project's [delivery capabilities](#delivery-capabilities), and neither reads or writes membership, roles or invitations.
...
This removal takes row locks on the project's administrators, which makes it likelier than most writes to lose a lock race — the wait bound it meets is the API's, carried by every transaction it opens — so it has the same second, non-invariant `409` as the workspace-level removal: [lock contention](#errors), carrying `Retry-After` and worth retrying **unchanged**. The last-administrator invariant carries no such header, because retrying it unchanged cannot help; neither variant carries an `errorType`.
...
| `DELETE` | `/admin/{catalog}/{id}?replaceWithId=` | Delete; `409` while issues reference it and no replacement is given — with `replaceWithId`, affected issues are remapped. The replacement is resolved among the entries **visible to this scope**, which here means global ones only: any other id — a workspace- or project-scoped entry included — is `404`, never `403`, so one you cannot see is indistinguishable from one that does not exist |
...
| `GET/POST` | `/admin/fields` | List custom fields (with usage counts) / create (`{"name", "key?", "type", "config?", "description?"}`; blank `key` is derived from the name — the type is immutable afterwards, and so is the key **except while a built-in search name has taken it**, which is what the `PATCH` row below renames out of). Create/rename returns `409` when the name **or** key collides with any field visible to the scope, inherited ones included (reuse it instead of duplicating), and the same `409` — one message for both unique constraints, `That key or name was taken by another change — reload and try again.` — is what a **lost race** answers, so a concurrent writer that takes the name or the key between the check and the insert is a conflict rather than a `500`. **Minting a key** has a second `409`: a key the [HQL search vocabulary](#search-hql) has claimed — a name the vocabulary itself registers, its spelling aliases included — is refused, because a search name always outranks a custom field of the same key, so such a field would work everywhere in the product except search, where the name means the built-in field (a field already sitting under such a key is [reported by `/search/schema`](#search-hql) under `shadowedFields`, and carries `shadowedBy` here). A [retired key](#search-hql) is refused too, by a rule of its own and with its own message. It is not claimed by the vocabulary and the mechanism is the reverse one — a retired key is resolved **last**, so a custom field keyed with one is not shadowed but *wins*, and the compatibility fallback stops firing for everyone that field is visible to: every saved filter written before the key was retired silently starts matching a custom field instead of the built-in that replaced it, and for a **global** definition that is every workspace on the instance at once. Both refusals are `409`, they say different true things, and neither implies the other. The check runs **after** the blank-`key` derivation, so a field merely *named* "Project" is refused just as `"key": "project"` is; it guards **both** doors that mint a key — create, and a rename's target — and it is never retroactive, since the rows that predate a registration are exactly the shadowed population the rename exists to let out |
| `PATCH` | `/admin/fields/{id}` | Update name/config/description, and — only while the field is [shadowed](#a-custom-fields-key-is-renameable-exactly-while-a-built-in-search-name-has-taken-it) — its `key`. `200` when there is no rename to do; `422` when a rename is requested outside that population; `409` for a system field, a registry-claimed target key, a target that already exists or is inherited, or a lost race; `404` for a field not at your scope. (Also `POST /{id}/archive` · `/unarchive`) |
...
| `GET/POST` | `/admin/users` | List accounts (paginated `?page=&size=`, oldest first) / create (`{"email", "displayName", "systemRole?"}`). No password or email — the `201` response is `{"user", "setupLink"}`; hand the one-time `setupLink` (`/reset-password?token=`, valid 7 days) to the person — it is a password-reset link like any other, so it is also retired the moment that account [completes any reset](#redeeming-a-reset-link-retires-every-other-outstanding-one); regenerate it below if that happens. `400` on the ordinary field bounds (blank/too-long `email` or `displayName`), and separately when the address, once lower-cased for storage, would not fit `users.email` — [see above](#validation-failures-400). `409` when the address is already registered (matched case-insensitively, exactly as on [`POST /auth/register`](#auth-endpoints)). On DC this is the primary way to onboard users |
| `POST` | `/admin/users/{id}/setup-link` | Regenerate the one-time setup link → `{"setupLink"}`. Issuing does not retire the link it replaces — earlier links stay usable for the rest of their seven days, until the account [completes any reset](#redeeming-a-reset-link-retires-every-other-outstanding-one) |
...
**A custom field's `config` is bounded, and so is what is inside it.** Serialized compactly the whole document may be at most **20 000 characters** — checked for every field type, not only selects; a select may carry at most **100 options**; and an option's `id` and `label` may each be at most **100 characters**. Each is a `422` naming what was measured against its limit, on create and on update alike, and on the [delegated](#delegated-administration) workspace- and project-scoped mirrors. The document ceiling is the binding one at the extreme, so it is the one to design against: 100 options each carrying a 100-character `id` *and* `label` is roughly 23 KB and is refused before the option checks are reached. What the ceilings protect is egress rather than storage — this `config` is re-served to every project member on every board load and every issue form, through [project configuration](#project-configuration).

**And a select option's `color` has to be a color.** It is `#RRGGBB` or `#RRGGBBAA`, [like every color in this API](#conventions); anything else — `red`, a three-digit shorthand, an empty string — is a `422` quoting the offending option's `id` and naming the shape it wants, on create and on update alike and on the [delegated](#delegated-administration) workspace- and project-scoped mirrors. This one is new: the value used to be stored and re-served entirely unchecked, on the endpoint every board load reads. Omitting `color`, or sending `null`, is unchanged and still means "no color". It is a **format** check and never a contrast check — the color is an identity hue and the text drawn on it is derived at render time, so `#FFFF00` is a perfectly good answer here. [Labels](#labels) apply the identical format to their own `color` and answer `400` for it, and the three taxonomy catalogs above apply its six-digit half and answer `400` too: one shape gets one sentence at every door — `Color must be #RRGGBB or #RRGGBBAA`, or `Color must be #RRGGBB` where the alpha form has nowhere to be stored — and only the status belongs to the endpoint. Those three catalogs are the change here: a refused swatch used to come back as the bare regular expression, and now names the shape in words. One thing not to read into the shared format, since it decides an answer: **neither door accepts a padded value**. `" #0EA5A4 "` is refused at both — `400` on a label, `422` on an option. Where the same value is checked in two places only the first one ever answers, and on this API a constraint declared on a request field is checked **before** the service behind it — so wherever the two could disagree, the annotation is what a client meets. An option's `color` has no annotation to meet, because it lives inside `config`, which no field constraint reaches into; there the service's own check is the first one and the only one.

Integrity rules: deletions never leave dangling references (remap or `409`), no workflow can end up empty, every priority set keeps a default, and a workflow change is refused while it would strand issues in statuses invisible to the board. The first of those is no longer only a rule these endpoints apply: since 0.17.0 the database enforces it too, so an issue cannot point at a status or issue type that does not exist by any route at all — a delete on a path that never checked, and an issue written against an entry deleted a moment earlier, are both refused rather than half-applied, with [`REFERENCE_CONSTRAINT_VIOLATION`](#errors).

### A custom field's key is renameable exactly while a built-in search name has taken it

A key normally never changes: [saved filters](#saved-filters) are stored as text and refer to fields by key, so renaming one would silently change what every stored query matches. There is one population where that argument does not hold — a field whose key the [HQL search vocabulary](#search-hql) has claimed. Such a field is unreachable from HQL both before and after a rename (`labels = "x"` answers from the built-in `label` field either way, because the vocabulary still claims the name), so nothing any saved filter means can change. That is the whole licence for this, and it is why the rename is offered *only* there: it is the affected tenant's exit, not a general capability. Once the field is out of the shadow its key is fixed again, so renaming it back is refused.

**`PATCH …/fields/{id}` treats `key` as a rename request — and the trigger is *difference*, never presence.** A `key` that is absent, `null`, blank, or equal to the field's current key (compared case-insensitively) renames nothing and **refuses nothing**: a client that echoes the current key back on every update keeps getting `200`, and retrying a timed-out rename gets `200` rather than a spurious conflict. Blank on update never means "re-derive from the name" either — that would rename a field silently whenever somebody edited its display label.

The refusals are evaluated in a fixed order, so the common mistake is answered by the rule that explains itself rather than by a message about the target key:

| Status | When |
|---|---|
| `400` | `key` is outside `[a-z0-9_]*` or longer than 50 characters — ordinary bean validation, and **this is the door an upper-case key hits**: `Team_Labels` is a `400`, never a `409`. `name` and `description` have a shape too, here and on create alike, since both doors take the same body. `name` is a **single line**: a line terminator (LF, CR, TAB, NEL, LINE/PARAGRAPH SEPARATOR) or any invisible or reordering character — zero-width, bidi mark or override, interlinear annotation, Unicode tag — is a `400` reading `Name must not contain control characters`. `description` is prose and keeps its line breaks, but it is **not otherwise exempt**: it takes the identical class minus TAB/LF/CR, so everything invisible or reordering is still a `400`, reading `Description must not contain control characters` |
| `404` | the field is not resolvable at your scope — including a workspace admin reaching for a **global** shadowed field, which is `404` and not `403` because the row is not at their scope; only a system admin can rename that one, and doing so changes the key in every workspace on the instance at once |
| `403` | a proven workspace member without [`workspace.taxonomy.manage` / `project.taxonomy.manage`](#permissions) on the [delegated](#delegated-administration) mirrors |
| `409` | the field is a **system field** (`isSystem: true`) — `System fields cannot be renamed.` Checked before the shadowing rule, so a claimed-key system field is never told it may rename |
| `422` | the field is **not** shadowed — renaming is offered only as an escape from shadowing |
| `409` | the **new** key is itself claimed by the search vocabulary (`labels` → `components` would walk out of one shadow into another), with the reserved-search-name message create uses verbatim |
| `409` | the **new** key is a **retired** search name (`story_points`, `fix_version`) — a different rule with a different sentence, because minting one does not shadow the field, it takes the name over and changes what pre-retirement saved filters match |
| `409` | the new key already exists or is inherited at this scope — checked **wider than the scope** (global ∪ ancestor workspace ∪ own project, the same reach create uses). The widening is **upward only**, which is exactly the reach create has: a *project*-scoped field inside your own workspace does not occupy the key at workspace scope, so renaming a workspace field onto a key one of its projects already uses currently succeeds, and that project then sees two definitions under one key. Known, and tracked as **HD-284** — unchanged by this endpoint, because create has the same reach and narrowing it here alone would change what create means |
| `409` | a concurrent writer took the key or the name first — the lost race on the unique constraint is translated, not a `500` |
| `200` | renamed, or no rename was requested |

**A rename moves no rows.** Values live in `issue_field_values` keyed by field **id**, field-set membership is keyed by field id, and issue history records the display *name* — so values, set placements and history all survive untouched. What moves with the key: the HQL name that reaches the field, its `/search/schema` entry (it leaves `shadowedFields` and enters `fields`), its `CUSTOM:<key>` value-suggest token, and the `key` in [project configuration](#project-configuration). What never moves: **saved filter text** — nothing rewrites it, ever, so anyone still querying the old key has to edit their filter by hand. That last part is the tenant's half of the exit and no endpoint can do it for them.

**`shadowedBy` is populated for archived rows too, and every *warning* surface is narrower than that.** The field response carries `shadowedBy` whenever the key is claimed, archived or not, because the rename has to be offered for an archived row as well — an archived definition is out of resolution entirely, so the licence for a rename holds even harder there, and rename-then-unarchive is the ordinary recovery path for a field somebody archived *because* it had stopped working. The surfaces that *warn* — `/search/schema`'s `shadowedFields`, the console's warning chip, the operator log line — use the live-only predicate (`shadowedBy != null && !archived`) instead, so a clean instance never warns about its own archived seed placeholders.
...
- **Reuse over duplication** — creating or renaming a status / priority / issue type / field (fields also on `key`) is refused with `409` when the name collides with any item **visible** to the scope, whether it's an own-scope row or one inherited from a wider scope (a system/global item, or a workspace item inherited by a project). Bind the inherited item instead of minting a scoped duplicate. A field's `key` carries one more `409`, identical at every scope, and it applies to every key the scope **mints** — on create, and as the target of the [key rename](#a-custom-fields-key-is-renameable-exactly-while-a-built-in-search-name-has-taken-it) a `PATCH` allows while a built-in name shadows the field. A minted key may be neither a name the [HQL search vocabulary](#search-hql) has claimed nor one a release [retired](#search-hql) — two rules with two different messages. Checked after the blank-`key` derivation, and never retroactive: a row that predates a registration keeps its key and is exactly the population the rename exists to let out — see [System administration](#system-administration).
...
- **A delete’s `replaceWithId` resolves the same way, and refuses differently**: a replacement not visible to the scope is `404` (`Replacement … not found`), not `422` and not `403` — an id belonging to another tenant is indistinguishable from one that does not exist. The same holds in the system console, where visible means global, so a global entry can only be replaced by another global one.
...
| `PATCH` | `{base}/{catalog}/{id}` | Update an own-scope row (for a field, this is also the [key rename](#a-custom-fields-key-is-renameable-exactly-while-a-built-in-search-name-has-taken-it), permitted only while a built-in search name shadows it) |
| `POST` | `{base}/{catalog}/{id}/archive` · `/unarchive` | Archive / restore an own-scope row |
| `DELETE` | `{base}/{catalog}/{id}?replaceWithId=` (fields: `?dropValues=`) | Delete an own-scope row; `replaceWithId` must name an entry visible to this scope (global ∪ workspace ∪ own project) — anything else is `404` |
...
**Onboarding users (DC).** Since self-registration is closed by default, create each account via `POST /admin/users` and share the returned `setupLink`. The recipient opens it, chooses a password (`POST /auth/reset-password` under the hood), then signs in normally. No SMTP is required for this flow. A setup link is one of the account's [outstanding reset links](#redeeming-a-reset-link-retires-every-other-outstanding-one), so it stops working as soon as that account completes any password reset — if the recipient got there through a forgot-password email first, they already have the password they chose and need nothing further; otherwise regenerate the link with `POST /admin/users/{id}/setup-link`.
...
`@DisplayName` mentions in a comment body notify the mentioned workspace members. `body` is capped at **10 000 characters**, the same bound every block of prose this product stores carries — see [validation failures](#validation-failures-400).
...
{ "status": 409,
  ...
  "quotaBytes": 10737418240, "usedBytes": 10510925824, "availableBytes": 226492416, "fileBytes": 13002342 }
```

**It is deliberately not a `413` and deliberately not a `429`, and the second distinction is the one that bites.** `413` is about *this* file being too large, and the refused file is very often well inside the per-file limit — a one-byte file would be refused just the same; `413` is also already the answer to both ceilings above, and two refusals with opposite remedies must not share a status *and* a shape. `429` would promise that waiting helps. **It never does: a quota does not refill, this refusal carries no `Retry-After`, and a client that retries it the way it retries a `429` loops for ever.** The only thing that changes the answer is fewer stored bytes. So sort the `409`s this endpoint can produce in this order: `Retry-After` present means [lock contention](#errors) and is worth retrying unchanged; otherwise `errorType: "STORAGE_QUOTA_EXCEEDED"` means the workspace is full and retrying is pointless; anything else is the archived project, which carries neither.

The four byte figures are there for every caller who reaches the refusal, so a client can say what is wrong without a second request — they are one workspace-wide aggregate with no per-project resolution, and the [breakdown that names projects](#workspace-storage) is a different endpoint behind `workspace.edit`. `availableBytes` is clamped at 0, `fileBytes` is the **parsed** size rather than a client-declared `Content-Length`, and the sentence in `detail` deliberately prescribes no action, because this refusal has no reader who is guaranteed to be able to act on it. Nothing is written when it fires: no attachment row, no stored file, no change to the counter — the check runs before a single byte reaches the file store. Read [`GET …/storage`](#workspace-storage) to show the position *before* the upload instead of after the refusal.

**Uploading also spends two [per-principal budgets](#rate-limits)** — the write budget shared by every mutating call under `…/projects/*/issues/**`, and an upload-**byte** budget summed over the parsed sizes of your own files — each of which answers `429` + `Retry-After` and each of which really is fixed by waiting. Deleting an attachment spends the write budget too, and frees its bytes immediately: the workspace is credited when the row goes, before the stored file is even touched.
...
- `color` is `#RRGGBB` or `#RRGGBBAA` (`400` otherwise — the [same format](#conventions) that a custom field's select option refuses with `422`, and the same sentence: `Color must be #RRGGBB or #RRGGBBAA`). Unlike `name` it is **not** normalized first — the constraint is declared on the field, so a padded `" #0EA5A4 "` is refused rather than trimmed. Omit it and the server picks a deterministic swatch derived from the name, so the same name always looks the same.
...
**The planning view is the largest single response this API produces, and it is bounded accordingly.** It spends the planning budget (240 requests a minute per user) and takes one place in the bound on how many expensive reads may run at once, so it can answer `429` either with an `errorType` or without one — see [Rate limits](#rate-limits) for telling them apart, and note that the two kinds mean different things by `Retry-After`. Both are spent before the project is resolved, so that `429` can arrive for a project you cannot see.
...
**Status codes.** `200` for a section, empty or truncated (an empty one is `issues: []`, `truncated: false`, `totalAvailable: 0` and zeroed `stats`). `400` for an unknown `labelMatch`, more than `MAX_LABELS_PER_ISSUE` distinct `labelId` values (20 by default), or a malformed `{sprintId}`. `404` for **every** tenancy failure, all indistinguishable from one another: unknown workspace, unknown project, a project of another workspace, a caller who is not a member, an unknown sprint, and a sprint belonging to another project or another tenant. `429` past either of the surface's two bounds — the planning budget, which carries no `errorType`, or the occupancy bound, which names itself in one (see [Rate limits](#rate-limits)); both are spent before the project is resolved, so a `429` here can arrive for a project you cannot see, and neither is ever a narrowed section. There is no `403` — beyond membership no permission gates a planning read.
...
**Both bounds apply here exactly as they apply to the whole view, and that parity is the point.** One registration covers the whole `…/backlog/**` path — the [per-principal planning budget](#rate-limits) of 240 requests a minute and the bound on how many expensive reads may run at once — so no planning read is cheaper to ask for than another. A half-budgeted surface would be worse than either whole answer, because a client refused on the cheap request retries with the expensive one.

**Which is the rule this endpoint puts on you: a `429` of any kind on a section refresh must not be answered by fetching `GET …/backlog`.** The whole view assembles every open section in a single transaction — several times the work of the section that was just refused — so escalating spends far more of the bound you have only just met. Keep the section you already have, mark it stale, and offer a person a manual retry. A client whose error path invalidates its cached view does exactly the wrong thing here without meaning to, which is worth checking in your own code: it is the obvious retry, and it is the one that hurts. A budget is earned by the work a handler does and not by where it is mounted, and a section fetch is not free either — it repeats that section's whole-section aggregation, and what it divides is the rows and the response.
...
The budget is spent **before** the workspace and project are resolved, as is every other refusal these interceptors make — which is where a `429` precedes a `404` in this API: a refused caller is refused even for a project they cannot see. That ordering is deliberate, and so is keying the budget on the **caller** rather than the project: a per-project key would answer differently depending on whether a project exists, turning a throttle into an existence oracle, and would let one colleague's open dashboard tab throttle everyone else on the project. Keyed on you, the `429` is identical for a real project, a nonexistent one and somebody else's.

**Being inside the budget is not the same as being admitted.** A report can also be refused because too many expensive reads are already *running* — yours, or the instance's — and that refusal is a `429` too, told apart by `errorType`, with a `Retry-After` that means something different. See [Rate limits](#rate-limits), which is where the causes and what to do about each of them are written down.
...
**`PROJECT` is a slice with no click-through.** Issues carry a project and your search scope already restricts which are visible, so the grouping is perfectly good. HQL now *does* have a [`project` field](#search-hql), but it resolves a project **key** while this slice is grouped and labelled by **name** — which resolves to nothing, project names carrying no uniqueness — so **every project bucket still ships `hql: null`**. The slice works; the drill-down does not, until the key is carried down to the fragment. That is a known gap with its own ticket, not a bug.
...
**There is no "matching issues" export in this API, and you should not read these files as one.** The "download matching issues" button is a different feature, and the search-side export path it needs **does not exist yet** — there is no CSV or export endpoint under `/search`, and it cannot be assembled client-side either: HQL can now scope a query to one project, but it still has no `resolved`/`closed` field, so the flow report's *resolved* half is not expressible as a query at all. It is filed as its own ticket. Until it lands, a report CSV is the numbers that were drawn and nothing else.
...
Caching is the family's (`private, max-age=60` + `Vary: Authorization`): a download is still a live read over one tenant's data. The throttle is the family's too — these six sit under `…/reports/**`, so they spend the same [reports budget](#how-every-report-behaves) as their JSON twins and sit inside the same [occupancy bound](#rate-limits), and a `429` is possible on any of them for either reason.
...
**How often you may ask is not the only bound on these paths.** How many of them may be *running at once* is bounded as well — your own in-flight requests, and the instance's — because a query runs its whole predicate twice and validating a saved filter builds the same resolution context `…/search/schema` pays for. Past that bound the answer is a `429` carrying an `errorType`, which the budget above does not carry, and whose `Retry-After` means something else; the causes and their remedies are in [Rate limits](#rate-limits).
...
| `GET` | `/workspaces/{wsId}/search/schema` | member | Autocomplete metadata: fields, per-field operators, value picklists, and `shadowedFields` (your custom fields whose key a built-in search name has taken — reported, never offered). **Suggestion-only, and [capability-aware](#search-hql)** — a field it omits still resolves |
...
- **Fields:** `status`, `assignee`, `reporter`, `type`, `priority`, `project`, `created`, `updated`, `due`, `closed` (alias `closedAt`), `parent`, `text`, `label` (alias `labels`), `component` (alias `components`), `fixVersion`, `affectsVersion`, `sprint` (alias `sprints`), `storyPoints` (alias `points`), plus any **custom field** by its `key` (e.g. `severity`). `/search/schema` advertises the fields your autocomplete should offer — that list is [capability-aware](#search-hql) and can be a **subset** of what actually resolves, so treat it as vocabulary, not as the definitive query surface. Field names are matched case-insensitively, so the all-lowercase `fixversion` / `affectsversion` spellings work too — `/search/schema` always advertises the camelCase name, exactly once per field. Whatever your query says, the scope stays all of your visible projects: a `project` term can only **narrow** it, never widen it, and every result row still names its owner (`projectId` / `projectKey` / `projectName`).
- **Operators:** `=` `!=` `IN` `~` `>` `<` `>=` `<=`. Which operators a field accepts is per-field (see `/search/schema`): `~` is a case-insensitive substring match, only on `text` (title + description); ordered comparisons (`>` `<` `>=` `<=`) apply to the date fields and to `priority`. Negation is the prefix keyword, for every field alike: write `NOT field IN (…)` — the grammar has no `field NOT IN (…)` form for any field at all.
- **Booleans:** combine terms with `AND`, `OR`, `NOT` and parentheses `( )`. Precedence is `NOT` > `AND` > `OR`.
- **Emptiness:** `field IS [NOT] EMPTY` for nullable fields (`assignee`, `parent`, `due`, `closed`, `label`, `component`, `fixVersion`, `affectsVersion`, `sprint`, `storyPoints`, and every custom field except CHECKBOX). On any nullable field, `!=` also matches the rows that hold **no value at all** — SQL's three-valued logic would drop them and the language deliberately does not, because "not closed on the 1st" plainly includes "not closed", exactly as `label != "needs-design"` has always included the issues carrying no label.
- **Sorting:** an optional trailing `ORDER BY field [ASC|DESC], …` clause — it must come last. Custom fields are **not sortable** (yet). Rows holding no value for the sort key land wherever the database puts them for that direction — last ascending, first descending — and that placement is the same for **every** nullable sort key, so no two of them disagree about which end the empty rows sit at.
...
- **Dates:** a date-valued field takes ordered comparisons (`= != > < >= <=`), **is sortable**, accepts `now()` and `startOfWeek()`, and does **not** accept `IN`. Its operand is a bare `YYYY-MM-DD` date that stands for the **whole UTC day**, inclusively: `created = "2026-08-01"` matches anything stamped from `00:00Z` that day up to — but not including — the next, `created <= "2026-08-01"` runs to the end of that day, and `created > "2026-08-01"` starts at the beginning of the following one. A time of day is never part of an operand.
- **Closed date:** `closed` is when the issue was last closed — the queryable form of the [`closedAt`](#issues) every issue response already carries, and a date field like the rest (ordered comparisons, sortable, `now()` / `startOfWeek()`, no `IN`), nullable. **`closed IS EMPTY` means the issue is open right now.** It does not mean the issue was never finished: the stamp is written when an issue enters a DONE-category status and **cleared** when it leaves one, so a reopened issue is empty again and the date it used to carry is gone — cleared, not archived. "Never closed" is therefore not a question this field can answer. By the nullable rule above, `closed != "2026-08-01"` also matches the issues that are not closed at all. The response spelling `closedAt` resolves too, as an alias, but `closed` is the name to write: it is the one `/search/schema` advertises, and this language names date fields after the **event** rather than after the column (`created`, `updated`), so spelling a single one of them after its column would leave the vocabulary unguessable in both directions. Example: `closed >= startOfWeek() AND assignee = currentUser()`.
- **Project:** `project` is single-valued and never empty, and is resolved by project **key** only — `project = "HD"`, matched case-insensitively with surrounding whitespace ignored, so a pasted `"hd "` resolves. Operators `= != IN`, and it **is sortable** (`ORDER BY project` orders by key). A project's **name** is deliberately not a resolution source: unlike a key, a name carries no uniqueness constraint, so a name operand would denote a *set* — the very ambiguity this field exists to remove. Expect this to differ from `component` / `fixVersion` / `sprint`, which do resolve by name. The name is not lost, it just moves to where being wrong is cheap: `/search/schema` and `/search/suggest` publish it as the **label** beside the key they insert (`{"label": "Hamstrack (HD)", "value": "HD"}`), and an operand that matches a visible project's name comes back with a hint naming that project's key (`Did you mean "HD" (Hamstrack)?`). Every issue has a project, so `project IS [NOT] EMPTY` is a `422` rather than a statement that could never be true. An operand naming no project you can search is the ordinary field-anchored `422` — and it reads **the same** whether the key is unknown, belongs to a project outside your reach, or belongs to an archived project. Those three are indistinguishable on purpose, so the field cannot be used to probe which projects exist; the message names the rule rather than this operand's fate (`No project with key 'OLD' that you can search. Archived projects are not searchable.`), which is also why archiving a project makes a stored [saved filter](#saved-filters) naming it fail at run time, exactly as an archived label or version does. Finally, this is the disambiguator the project-scoped catalogs never had: `project = "HD" AND fixVersion = "2.4.0"` says which "2.4.0" you mean.
...
- **Retired field keys:** **a key that a release retired resolves to the field that replaced it.** When a custom-field placeholder is promoted to a native field, the migration archives the old key and the replacement ships under a new name; the old key keeps resolving to that replacement, permanently, so a [saved filter](#saved-filters) written against it keeps running — and keeps *saving* — unchanged (nothing ever rewrites the stored text of a filter, and `POST`/`PATCH …/filters` re-validate whatever text you send them). That rule is what to rely on; the membership grows with every promotion, and as of this release it is `story_points` → `storyPoints`, `fix_version` → `fixVersion`, `labels` → `label`, `components` → `component`. Three consequences are visible to you. **Your own field wins:** a retired key is a last-resort fallback, consulted only after system fields and after the custom fields you can see, so a workspace that owns a custom field keyed `fix_version` reaches **its own field** and never the native one — which means the same query text legitimately means different things in two workspaces. That is the design rather than a bug: the alternative is the product taking a key away from every tenant already using it. **Errors name the canonical field:** once the fallback has applied, `story_points IN (5)` answers `422 "The IN operator is not allowed on field 'storyPoints'"`. **Not advertised is not the same as not accepted:** a retired key is absent from `/search/schema`'s field list and from every surface that enumerates the vocabulary, so a name that resolves perfectly well can be missing from the suggestion list — deliberate, not a defect. It is nevertheless **accepted** wherever a field name is accepted, `/search/suggest` included: `?field=fix_version` answers with the same values `?field=fixVersion` does (the suggester is chosen from the field the name resolved to, and the response echoes the name as you wrote it), and a retired key whose replacement has no value list of its own declines in exactly the words the replacement would. So a client holding a retired key can keep using it for typeahead as well as for filtering. It also used to mean a retired key was **not** a [reserved key](#system-administration), and that is the one part of this rule 0.18.0 changed: **every door that mints a key now refuses one** — `POST /admin/fields` and a [key rename](#a-custom-fields-key-is-renameable-exactly-while-a-built-in-search-name-has-taken-it) alike — with a `409` of its own, because a new custom field under a retired key is not shadowed by the fallback but *takes the name over* from it, and every filter written before the retirement then silently matches something else. Two rules reach these keys and they say different things: `labels` and `components` are *also* live built-in spellings (the plural aliases of `label`/`component`) and are refused as **reserved** search names, while `story_points` and `fix_version` are refused as **retired** ones. **Neither refusal is retroactive** — a workspace that already owns such a field keeps it and keeps resolving to it, exactly as described above, which is how the workspaces that own one came to.
...
- `/search/suggest?field=fixVersion` and `?field=affectsVersion` are scoped the same way — that endpoint is the overflow of the same `VERSION` picklist, so it could not honestly answer differently. No other field that endpoint serves is capability-narrowed.
...
**Schema** — `GET /search/schema` drives autocomplete: `fields` describes each queryable field (`name`, data-type `type`, allowed `operators`, `nullable`, `sortable`, `valueSuggest`, `functions`) — the system fields first, then your visible **custom fields** (`name` = the field `key`, `type` = its custom `FieldType`) — `keywords` lists the HQL keywords, and `values` holds the small picklists (`STATUS`/`TYPE`/`PRIORITY`) of names reachable by your visible projects, a `LABEL` picklist of the workspace's non-archived label names, a `COMPONENT` picklist of the non-archived component names of those visible projects, a `VERSION` picklist of the non-archived version names of those projects **that have `releases` on** (**one** picklist serves both `fixVersion` and `affectsVersion` — the two roles draw from the same catalog, so both fields declare `valueSuggest: "VERSION"`), a `SPRINT` picklist of the **open** (non-completed) sprint names of those projects **that have `board: SCRUM`**, a `PROJECT` picklist of those visible projects themselves — `{"label": "Hamstrack (HD)", "value": "HD"}` per entry, the human name shown and the key inserted, since the key is the only string `project` resolves — plus a `CUSTOM:<key>` entry per SELECT/MULTI_SELECT custom field (options as `{label, value=optionId}`). Any picklist drawn from a catalog a workspace can grow without limit — the label, component, version, sprint and project ones — is **capped at 200 entries**; the status/type/priority lists are not, being bounded by the taxonomy itself. Past a cap, fall back to the matching `/search/suggest?field=…` (`label`, `component`, `fixVersion`, `project` are served there). **A picklist whose catalog is bounded by construction has no `/search/suggest` fallback at all** — today that is `sprint` (`?field=sprint` returns the usual `422`), because a project's open sprints are capped and cannot realistically overflow the picklist. `storyPoints` has no picklist at all — it is numeric — and a date field declares `valueSuggest: "DATE"` while `values` holds no `DATE` entry, because a date is typed rather than picked. The member list (including USER custom fields) is deliberately **not** embedded — use `/search/suggest` for it. The system-field half of `fields`, and the `SPRINT` / `VERSION` picklists, are **capability-aware** (above): a field or a name that is missing here is still perfectly queryable.

**`shadowedFields` — the fields this endpoint used to drop in silence.** A registered search name outranks a custom field of the same key, so a workspace that owns a field keyed `labels` cannot reach it from HQL: `labels = "x"` answers from the built-in `label` field. That field is now listed in `shadowedFields` as `{key, name, shadowedBy}` — sorted by key, `[]` when there are none — where `shadowedBy` is the **canonical** built-in name claiming the key (a key of `labels` reports `label`, the field whose data the query actually answers from). It is a **separate list and deliberately not part of `fields`**: every entry in `fields` is a name that means what the entry says it means, and a shadowed key listed there would have clients autocomplete a name that answers from the built-in field's data rather than from the custom field. Only your own visible fields appear (the list is built from the same resolution context the rest of the response is — no extra query, no cross-tenant read), archived definitions never do, and **no status code changes**: this endpoint is `200`, or `404` for an unknown workspace or a non-member, exactly as before. The exit is a [key rename](#a-custom-fields-key-is-renameable-exactly-while-a-built-in-search-name-has-taken-it), permitted exactly while the field is listed here; afterwards it leaves `shadowedFields` and enters `fields` under its new key, while the old key keeps meaning the built-in field.
...
```json
{
  ...
  "values": {
    ...
    "PROJECT": [ { "label": "Hamstrack (HD)", "value": "HD" } ],
...
…and **projects** — `field=project`, over the projects you can see. Here `q` is a **prefix**, matched against the **key** and the **name** alike, since someone typing `HD` and someone typing `Hams` are after the same project; the `value` is always the **key**: that is the only string the `project` field resolves, and the human name rides along in the `label`. Archived projects are never offered — not here and not in the `PROJECT` picklist this overflows — because a value surface may only offer what resolution accepts. Unlike the version fields, this one is not capability-narrowed: every project has a project.

```json
{ "field": "project",
  "suggestions": [ { "label": "Hamstrack (HD)", "value": "HD" } ] }
```
...
**A [retired key](#search-hql) is accepted here too.** A field name resolves exactly as it does in a query, so `field=fix_version` answers as `field=fixVersion` does — same values, same scoping, your own spelling echoed back in `field`. Such a key is not named among the fields above and never appears in `/search/schema`, because it is compatibility rather than vocabulary; that is a statement about what is *advertised*, not about what is accepted.
...
**Every endpoint here is on the search surface's bounds**, `GET` and `DELETE` included, since the binding is by path and not by method: the per-principal budget of 120 requests a minute, and the bound on how many expensive reads may be running at once. Validating a filter's HQL is search-surface work wherever it is mounted, so both refusals are `429` and `errorType` says which — see [Rate limits](#rate-limits).
...
## Browser violation reports (CSP)

```
POST /security/csp-report
```

> **This endpoint does not exist on a default self-hosted installation.** Report collection is **off** unless the operator sets `CSP_REPORT_SINK_ENABLED=true` ([Operator settings](#operator-settings-that-affect-the-api)) — so unless yours was configured for it, this path answers `405`, and the policy header the instance sends names no `report-uri` at all. An endpoint being there because the software has it is the rule in this reference; this one is there only if somebody switched it on, so check your instance before you build on it. **Serving the endpoint and naming it in the header are one setting**, deliberately: there is no configuration in which the header points at a path the instance does not serve. Leaving it off is a complete answer rather than a gap — the report-only policy still ships, so a violation still appears in the console of the browser that hit it.

**Nobody calls this endpoint by hand, and no integration should.** Where it is served, every response the instance sends carries a `Content-Security-Policy-Report-Only` header whose `report-uri` names this exact path, and when a page breaks the policy the browser posts a report here by itself. That is the entire caller list, and reading the statuses below as answers to a *person* is what makes them look strange. The sink stores nothing, hands nothing back, and is not workspace-scoped — a report is emitted by a browser rather than by a session, and carries no tenant identity that could be trusted if it did. It is documented because it is part of the public HTTP surface.

**Unauthenticated, by necessity.** A browser sends a violation report with no credentials, so requiring any is requiring a report nobody can send. What makes that safe is that the endpoint can only ingest: there is no read path behind it and nothing it accepts or returns is tenant-derived. The exemption is scoped to `POST` on this one path — every other method stays inside the authenticated `/api/**` set. It is nevertheless an unauthenticated public door, which is why an operator turns it on deliberately; `docs/self-hosting.md` has the pre-flight, including pairing it with `RATE_LIMIT_TRUST_FORWARDED_FOR` so the per-address budget still keys on a real client address behind your proxy.

**What it accepts.** `Content-Type` must be `application/csp-report` (what a `report-uri` sends), `application/json` (what several tools send) or `application/reports+json` (the Reporting API's). The body may be a bare report object, the `{"csp-report": {…}}` envelope, a Reporting API entry (`{"type": …, "body": {…}}`), or an array of any of those. **At most 20 reports are read from one body** and the rest are discarded; the body itself is bounded at **16 KB**. The volume budget below is spent **per report recorded, not per request**, so a batch costs what it writes and the per-minute numbers are reports rather than requests.

**`204` is the only answer a report ever gets, and it does not mean the report was kept.** A report that was parsed and logged, one dropped because the page it names belongs to another site, a body that was not JSON at all, and everything past the twentieth report in a batch are all answered `204` with an empty body. Nothing in the response — status, headers or body — says which happened, and that is the design rather than an oversight: a discriminating answer is a free oracle about the instance, including an answer to "is this sink watching?". Read the `204` as *the request was well-formed enough to hand over*, never as *accepted*. Everything below happens **before** the handler — in the security chain, the mapping, or the guard filter in front of both — so each one is a refusal about the request and never about the report.

| Status | When |
|---|---|
| `204` | The request reached the handler — report kept, dropped or unreadable alike. Empty body |
| `401` | Any method other than `POST`, without a token. The anonymous exemption is `POST`-scoped, so a `GET`, `PUT` or `DELETE` here is an ordinary `/api/**` request; nothing about the sink refused it, and the answer says nothing about whether the sink exists |
| `413` | The declared `Content-Length` is over 16 KB, refused in a filter before the body is read — a cap enforced after a body has been read has already paid the cost it exists to avoid. A body that declares no length at all is bounded too, but by the handler, which stops reading at the ceiling and discards the excess, so *that* one is a `204`. No body |
| `415` | The content type is not one of the three above |
| `429` | Over the sink's own volume budget — 60 reports a minute from one client address, 600 across the instance (`CSP_REPORTS_PER_MINUTE_PER_IP`, `CSP_REPORTS_PER_MINUTE`). `Retry-After` in seconds, never above 60, and no body. It is not the auth budget and never spends it: see [Rate limits](#rate-limits) |
| `405` | **The default**: this instance is not collecting reports. It is a `405` rather than a `404` only because the SPA fallback maps every dotless path for `GET`, so the dispatcher finds the path and refuses the method — the answer to the question that matters, "is there a sink here", is the same either way |

**What an accepted report becomes.** One log line on the instance and two counters (`hamstrack_csp_violations_total`, `hamstrack_csp_reports_dropped_total`) — no table, no row, no file, and nothing readable back through this API or any other. The URLs inside it are reduced before they are logged: the reporting page keeps its path and loses its query, because in this product that query can be an [HQL search](#search-hql) — the text of somebody's issue titles — and a report about a page this instance does not serve is dropped rather than logged, which is what stops a public sink being anybody's journal. What collection costs you is log volume, and that is the thing to size before turning it on.

---

*This reference will grow with the product. Found a mismatch between docs and behavior? Please [open an issue](https://github.com/Zherikhov/hamstrack/issues).*
