...
| `429` | Rate limited — wait the number of seconds in the `Retry-After` header. **Where the refusal is spent differs by surface**: the per-principal report and search budgets are spent before the resource is resolved, so their `429` can precede a `404`; the [invitation ceilings](#invitations-are-throttled) are spent after it, so a workspace you cannot see answers `404` even when you are over one. See [Rate limits](#rate-limits) |
...
**Asking too often is not the only way to be refused on that surface.** Those same paths — every [report](#reports), the whole of `…/search/**` and `…/filters/**`, and the [storage breakdown](#workspace-storage) — are additionally bounded by **how many of their requests may be running at once**: one bound on the caller's own in-flight requests, and one on the whole instance's, so that the rest of the API always keeps a share of the database connections however busy this surface gets. It is not a rate and it does not refill: what clears it is something finishing. Nothing is computed when one is refused, so the identical request is safe to repeat once there is room, and the refusal is a `429` like the budgets above.
