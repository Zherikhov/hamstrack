...
### 12.1 Bench
Eleven project-native agents, each with explicit `model:` / `effort:` frontmatter (never override per call): builders `backend-builder`, `frontend-builder`; reviewers `tenancy-reviewer`, `security-officer`, `ops-reviewer`, `dc-cloud-guard`, `migration-reviewer`; verifiers `test-runner`, `browser-qa`; `systems-analyst`; `api-docs-sync`. **The values live in the frontmatter and nowhere else in prose** — this paragraph deliberately does not restate them, because the copy that used to stand here went stale on 2026-09-09 and again on 2026-09-10 while every reader believed it. `AgentChecklistFreshnessTest` holds the frontmatter equal to CLAUDE.md § Subagents (the one mirror the bench design keeps, because a router needs the pair at a glance); a change to either is dated in `docs/retro/README.md`. No imported or generic agents exist; the routing invariant now holds by construction.
