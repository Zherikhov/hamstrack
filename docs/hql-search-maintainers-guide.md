...
| **`ratelimit/**`** | **Two controls, one registration.** `SearchRateLimitConfig` binds them to **paths**, not handlers: `/api/workspaces/*/search/**` **and** `/api/workspaces/*/filters/**`. (a) **The search budget** — `SearchRateLimiter`, per principal, `app.search.requests-per-minute`, default 120: how *often* one caller may ask. Past it: `429` + `Retry-After`. (b) **The expensive-read occupancy bound** (HD-182) — `ExpensiveReadConcurrencyLimit`, `app.expensive-read.max-in-flight`: how many requests may be *running at once*, per principal and across the whole surface, as a counted share of the connection pool. Past it: `429` with `errorType` `TOO_MANY_IN_FLIGHT` or `EXPENSIVE_SURFACE_BUSY` and `Retry-After: 1`. The second exists because the first cannot deliver it: a rate spends the same unit whether a request takes 8 ms or 8 s, so its protection evaporates exactly as the instance slows down. **This is why a new endpoint under either path gets BOTH the moment its mapping exists** — and why one mounted anywhere else gets neither. See the cookbook entry in §6 before adding a surface. |
...
- **Anywhere else → it is unthrottled AND unbounded**, and nothing will fail until it is being abused. Add the path to a limiter config explicitly. The standing example is the Insights panel: it is a report, but it lives on the search path, so it inherited neither budget by resemblance and is bound to *both* explicitly (`ReportRateLimitConfig.INSIGHTS_PATH` and `SearchRateLimitConfig.SEARCH_PATH`). The occupancy bound rides on the same registration on purpose — there is no second pattern list to forget.
