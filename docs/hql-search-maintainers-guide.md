...
| `SearchController.java` | `POST /search`, `GET /search/schema`, `GET /search/suggest`. `@Valid` request; `@Size` on params. **Every endpoint here is throttled and occupancy-bounded** — not by anything in this class, but by a path binding in `ratelimit/` below. |
...
| `filter/**` | Saved filters (HD-26) — see §7. Also on the search budget *and* the expensive-read occupancy bound — see `ratelimit/` below. |
| **`ratelimit/**`** | **Two controls, one registration.** `SearchRateLimitConfig` binds them to **paths**, not handlers: `/api/workspaces/*/search/**` **and** `/api/workspaces/*/filters/**`. (a) **The search budget** — `SearchRateLimiter`, per principal, `app.search.requests-per-minute`, default 120: how *often* one caller may ask. Past it: `429` + `Retry-After`. (b) **The expensive-read occupancy bound** (HD-182) — `ExpensiveReadConcurrencyLimit`, `app.expensive-read.max-in-flight`: how many requests may be *running at once*, per principal and across the whole surface, as a counted share of the connection pool. Past it: `429` with `errorType` `TOO_MANY_IN_FLIGHT` or `EXPENSIVE_SURFACE_BUSY` and `Retry-After: 1`. The second exists because the first cannot deliver it: a rate spends the same unit whether a request takes 8 ms or 8 s, so its protection evaporates exactly as the instance slows down. **This is why a new endpoint under either path gets BOTH the moment its mapping exists** — and why one mounted elsewhere inherits nothing *from this registration*. It is not therefore unprotected: `ReportRateLimitConfig`, `PlanningRateLimitConfig` and `WriteRateLimitConfig` bind their own paths the same way, and what an endpoint gets is decided by which pattern list names it, never by resemblance. See the cookbook entry in §6 before adding a surface. |
...
**Check the throttle. It is inherited by path, never by resemblance — and there are two of them.**

- Under `/api/workspaces/*/search/**` or `/api/workspaces/*/filters/**` → **nothing to do**: `SearchRateLimitConfig` already covers it, and it gets both the per-minute budget and the expensive-read occupancy bound the moment the mapping exists.
- **Anywhere else → it inherits neither of these two**, and unless some *other* registration names its path — `ReportRateLimitConfig`, `PlanningRateLimitConfig` and `WriteRateLimitConfig` each bind their own, and the authoritative set is the one `ThrottleCoverageTest.theThrottledPathSetIsSealed` prints when it fails — it is unthrottled AND unbounded, with nothing failing until it is being abused. Add the path to a limiter config explicitly, and check which config rather than assuming this one. The standing example is the Insights panel: it is a report, but it lives on the search path, so it inherited neither budget by resemblance and is bound to *both* explicitly (`ReportRateLimitConfig.INSIGHTS_PATH` and `SearchRateLimitConfig.SEARCH_PATH`). The occupancy bound rides on the same registration on purpose — there is no second pattern list to forget.
- **Ask the occupancy question separately from the rate one**: does this handler hold a connection while it works? A rate budget bounds how often it may be asked for and nothing at all about how many may run at once, which is how one caller inside their documented allowance saturated a whole replica (HD-182).
...
- **API:** `/api/workspaces/{ws}/filters` CRUD. `SavedFilterNameBlankException` → 400, `…Conflict` → 409, `…NotFound` → 404, and **`429` + `Retry-After`** on every operation (`GET` and `DELETE` included) once the caller is past the search budget *or* past the expensive-read occupancy share — the binding is by path, so it is method-agnostic and carries both controls. Saved filters are on the *search* budget rather than a pot of their own because **validating a filter’s HQL is search-surface work**: `validateHql` builds the same `ResolutionContext` `GET …/search/schema` pays for (~8 statements, incl. a workspace-wide label projection and a full member scan), so `POST …/filters` with a deliberately invalid body was an unthrottled 8-query refusal loop.
