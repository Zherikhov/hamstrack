...
| `hamstrack_auth_signup_refused_total` | counter | `reason`(signup_closed/terms_not_accepted/published_password/unencodable_password/email_too_long) | `POST /api/auth/register` refused **before it spent a bcrypt or a mail ceiling** (HD-261; phrased over the spends rather than over the address since HD-306, when a refusal about the address joined): the door is closed (`app.registration.public-signup-enabled=false`, the DC default — it answered 403 for months with no witness at all), the request did not accept the terms, the password is one this repository publishes (`DataSeeder.PUBLISHED_PASSWORDS` — the reason with security value: somebody is trying a credential they read here), the password is over 72 UTF-8 bytes and BCrypt could not hash it, or `email_too_long`: the address is at most 255 characters as typed and over 255 once **lower-cased**, so `users.email VARCHAR(255)` could not hold it (`toLowerCase` maps U+0130 to two code points, so 64 of them plus a 190-character ASCII domain is 255 raw and 319 stored — measured). Until HD-306 that request answered the `22001` backstop's 400 **with an ERROR line**, after a bcrypt-12 (~370 ms) and a recipient-ceiling spend inside an advisory lock. The two password refusals are shared with the reset door, and the address gate is shared with the admin-console and invite doors; all of them are counted by register only — a reset is not a signup, and neither is an administrator's typo. **A knock on a closed door is information, not an incident — there is deliberately no alert rule**; this row and the counter are its witness. Status codes are unchanged: a counter is not an oracle |
...
| `hamstrack_scheduled_job_last_run_timestamp_seconds` | gauge | `task`(`Class#method`, one per scheduled task — a dozen or so, bounded by the tree) | Epoch seconds of the last **completed** run of one in-process scheduled task, success or failure (HD-298). **Every** `@Scheduled` method gets it with no edit: Boot 4 observes each run as `tasks.scheduled.execution` (measured 2026-09-09 — package-private `void sweep()` included) and `ScheduledJobHeartbeat` is a handler on that observation; a task handed to the registrar as a plain `Runnable` is invisible to that and is wrapped once at its site (`heartbeat.wrap`, `StorageReconcileSchedule`) — `ScheduledJobHeartbeatTest` refuses a registered task that is neither. **Seeded with process start, never zero** (the HD-202 rule): a task that never runs once ages from boot. The label is `task`, **not `job`** — Prometheus owns `job` on every scraped series and, with `honor_labels` off, renames an exported one to `exported_job`, so a rule written against `job` matches nothing. Per replica: `max by (task)`, never `sum`. `ScheduledJobStale` reads it against the period below |
| `hamstrack_scheduled_job_period_seconds` | gauge | `task` | The task's own period — a fixed delay/rate, or the gap between two cron fires — so one rule can compare age against schedule per task instead of one threshold per job. `0` means the trigger is not one the heartbeat can read (a custom `Trigger`; none today), which collapses the stale bound to its 900 s allowance: such a task pages early rather than never, and a WARN at registration says so |
| `hamstrack_scheduled_job_failures_total` | counter | `task` | Runs that ended in an exception. Spring logs those at ERROR and reschedules, and **`last_run` advances on a failed run too** — deliberately, because "ran and threw" and "did not run" have different first moves — so this is the only number that sees a job failing every time. Registered at zero for every task at boot, so `increase()` sees the first failure. `ScheduledJobFailing` fires on any |
| `hamstrack_workspaces_created_total` | counter | `source`(user/demo) | workspaces created. An `onboarding` value was declared and never emitted (onboarding creates no workspace of its own) and was deleted by HD-298 — `OpsWitnessContractTest` refuses a label value no production line names |
...
| `hamstrack_mail_dead_letter_skipped_total` | counter | `reason`(best_effort/pool_contended/hourly_cap/write_failed/in_flight_interrupted/foreign_task) | A message that will never be sent got **no `failed_email` row**, and why (HD-234). The row above counts the *loss*; this counts the loss of the *record* — the degradation from durable to loud that used to be one ERROR line per hour and no number. The category is **every branch in the mail package that ends without a row** — `MailService` (retries exhausted on best-effort mail; the dead-letter INSERT failed), `UndeliverableMail` (never attempted) and the pool's shutdown drain — each emitting **inside the branch that decided it**, so the counter and the log line cannot disagree; `OpsWitnessContractTest.DROP_SITES` names each branch and scans it for the call. `best_effort` is by design (an invite earns no row, ADR-0021) and `MailDeadLetterSkipped` excludes it. **Which reasons the alert actually sees:** `pool_contended` (the pool was already queued, `FailedEmailWriter` refused a second connection), `hourly_cap` (`app.mail.dead-letter.max-never-attempted-per-hour` — the queue is saturated, look at the pool and SMTP before raising it) and `write_failed` from a running instance (`UndeliverableMail.record`, `MailService.deadLetter`) — those three and no others. `in_flight_interrupted` (a drain expired with a worker inside a send — no row is possible, an upper bound), `foreign_task` (something not a `MailTask` reached the pool, see `AsyncConfig`) and the drain's own `best_effort` / `write_failed` (`recordAll`) are counted **during bean destruction**, after `ContextClosedEvent` has closed the management child context that is the only scrape target (`AbstractApplicationContext.doClose` publishes the event and runs `LifecycleProcessor.onClose` before `destroyBeans` — spring-context 7.0.8, read from the bytecode), so those series never exist on any scrape and no rule can see them. They are counted for symmetry with the category; their witness in effect is the log line beside each count — Loki search strings `shutdown drain expired` (both drain WARNs), `were not MailTasks` (the foreign-task ERROR) and `never-attempted dead-letter row` (the residue INSERT failed; `UndeliverableMail.recordAll`'s line is the one saying `those emails are lost`, and the same words also hit a running instance's single-row write failure and the hourly-cap summary). `NOT dead-lettered` is **not** a drain string — it is a running instance's refusal in `UndeliverableMail.record` (`pool_contended` / `hourly_cap`), two of the reasons the alert already sees. No rule is log-backed yet; that is a separate decision |
| `hamstrack_mail_stored_address_truncated_total` | counter | `column`(recipient_key/recipient_email/failed_email_recipient) | A stored copy of a recipient address did not fit the 320-wide column that holds it and was cut to that width at the site that writes it (`MailAddresses.fitStoredMailValue`, HD-306) — two columns on `mail_send_events` and, since the fix loop, `failed_email.recipient`, whose cut was an uncounted `substring` while the category sentence covering all three claimed each was counted. **Both `mail_send_events` columns are written from the same caller-folded address, one line apart in `RecipientMailThrottle`, and both can overflow** — HD-306 bounded them in two rounds, which is why they share one meter and one witness shape rather than having two of each. `column="recipient_key"`: the value every ceiling counts, so a cut means **two inboxes now share one ceiling bucket** — the fail-safe direction for a ceiling (an extra match raises a count and refuses *sooner*), and still a slot spent by an innocent bystander who cannot see why. `column="recipient_email"`: the forensic copy, so a cut costs an operator a few characters of a domain suffix and costs no ceiling and no delivery. Two things cause either and both are worth a look: somebody is submitting pathological addresses, or the arithmetic those widths rest on has drifted again. **Neither width is bounded by the request DTOs**, and a claim that they were is what this ticket corrected twice: `@Size(max = 255)` bounds the RAW text, every door folds before storing, and `toLowerCase` APPENDS (U+0130 → two code points) — so the key reaches `128 + 1 + 255 = 384` on every DTO without an ASCII-only local part, and the stored address reaches **347**, both from values those DTOs accept with zero violations (measured 2026-09-13). The `64 + "@" + 255 = 320` arithmetic holds for `InviteMemberRequest` alone. Truncating rather than widening is deliberate: a `22001` **inside** the throttle rolls back the ceiling row the caller has already been counted on (a free probe of a stranger's ceilings) and, on `forgot-password` / `resend-verification`, changes the status of an endpoint whose whole contract is one uniform response. **The only label is the column** — the domain is unbounded cardinality, so it goes to the log line, **domain only** (Loki: `was cut to fit`), and that line follows `record`'s own level rule: `DEBUG` for an anonymous caller, `INFO` when there is a sender to name. **In this build every path that can reach a `mail_send_events` cut is anonymous, so the line is DEBUG and production runs at `LOG_LEVEL_APP=INFO` — do not go looking for it in Loki.** The one call site that passes a sender is `InviteThrottle.requireRecipientCeilings`, and that door can overflow neither column: its worst-case key is **320 exactly** and its worst-case folded address **85** (measured 2026-09-13 against `MailAddresses`), because `InviteMemberRequest`'s ASCII local-part `@Pattern` plus `@Email`'s 64/255 give exactly `64 + "@" + 255`. The INFO branch is therefore unreachable until a door with a sender can overflow a column, and `failed_email_recipient`'s WARN is unreachable today for a different reason (every source of a `MailTask` recipient is a 255-wide column). **So the operator-runnable witness is this counter plus the rows themselves:** `select count(*) from mail_send_events where length(recipient_email) = 320` (and `= 320` on `recipient_key`), subject to `AuthMailProperties.ANONYMOUS_EVENT_RETENTION` for anonymous rows, and `select count(*) from failed_email where length(recipient) = 320`. The counter is unconditional, and it is the only thing that can say a cut happened at all. **No alert and no panel by decision:** a cut is neither an error nor caller-visible, both non-`failed_email` columns are swept within a day, and a rule on the first occurrence would page on one pathological address; the reader is the query above, run from the post-deploy read-back in `docs/release-checklist.md` |
...
| `hamstrack_csp_violations_total` | counter | `directive`(default-src/base-uri/object-src/frame-ancestors/form-action/script-src/style-src/font-src/img-src/connect-src/frame-src/worker-src/**other**) | a browser reported a violation of the **report-only** Content-Security-Policy (HD-264). It is the instrument the enforcement decision will be made from, which is why it exists before anything is enforced. **A rise here is not an incident and there is deliberately no alert rule on it** — the same convention `report_requests` and `search_requests` follow: report-only blocks nothing, so a large count means the measurement is working. **The reading that IS a finding is a flat zero**, because a broken sink, a dropped header and a clean policy all look identical — which is why the docs page's Swagger validator badge is deliberately left in place as a pre-declared canary (an `img-src` violation with `blocked-uri` host `validator.swagger.io`, on every real hostname and never on `localhost`). `directive` is mapped through a **closed set**: the raw `effective-directive` arrives in the body of an unauthenticated POST, so passing it through would be an unbounded-cardinality write into this store from the public internet. Anything the policy does not ship is `other`; the full directive text survives in the log line, where it costs nothing. Reports are **never persisted** — Loki's retention is the history |
| `hamstrack_csp_reports_dropped_total` | counter | `reason`(foreign_document/too_large/unparseable/budget) | a report that arrived and was **not** counted as a violation. Its whole job is to keep the counter above honest: the sink answers `204` to everything, including a body it threw away, so without this a quiet violations graph could not be told from a foreign-origin flood filling the budget or from a body bound nobody noticed was too tight. `foreign_document` is somebody pointing their own site's `report-uri` at this instance — expected at a trickle on a public box, and a sustained rate means somebody is using the sink as a public log sink. `budget` means the per-IP or instance ceiling refused a sender (`CSP_REPORTS_PER_MINUTE_PER_IP` / `CSP_REPORTS_PER_MINUTE`, deliberately **outside** `RATE_LIMIT_ENABLED`). No alert rule, for the reason above |
...
| Metric | Type | Labels | Meaning |
|---|---|---|---|
| `hamstrack_config_drift` | gauge | `scope`(files/containers/installed-ops/edge-body-limit) | whether the box differs from what was deployed (`1`) or not (`0`). **`files`**: a synced file was edited, or a file was added to a synced directory, since the deploy. **`containers`**: `docker compose up -d --dry-run` — Compose's own plan for the command a deploy runs — would **act on** at least one container, i.e. the file on disk is right and the running container was never recreated from it. The same scope also reads `1` when a declared service has no container at all; when a container runs under a service none of the compose files THIS CHECK WAS POINTED AT declares (the plan deliberately omits the deploy's `--remove-orphans`, so those are read out of Compose's own orphan warning instead — a sweep flag has no place in a read-only monitor; and “none of the files this check was pointed at” rather than “no compose file” because a box that narrowed `COMPOSE_FILES` for the deploy and not for the timer is being shown its own healthy stack — the journal line therefore names the set it resolved and makes the deletion claim conditional on the two agreeing); and when the plan could not be read (a failed invocation, or a running container the plan does not mention): *a check that could not ask its question has not shown that the box is clean*. A dependency that is not healthy makes Compose abandon the plan, so a health incident outlasting the alert's `for:` raises this scope too — the journal line says so, and it clears when the health does. Since HD-221 — it used to compare `com.docker.compose.config-hash` labels against `docker compose config --hash`, which disagreed with `up -d` on production permanently, for a cause that was never established and stays open. **`installed-ops`**: an artefact installed under `/usr/local/bin` or `/etc/systemd/system` differs from its copy in `/opt/hamstrack/ops` — a deploy places files and never installs them, so this scope exists to say that the gap is open. **`edge-body-limit`**: the deployed `Caddyfile` carries no `request_body`/`max_size` block (HD-191) — the one scope that compares the box against the repository's *intent* rather than against what a deploy placed, because the applier hard-refuses to sync a `Caddyfile` and that block therefore arrives only by a hand merge; until it does, an over-sized upload is refused only after the app has read the whole body, on the one lane no rate-limit budget bounds. A box with **no** `Caddyfile` is not running the bundled proxy and reads `0`. *Which* files differ goes to the journal, never into a label |
...
| `hamstrack_deploy_verify_check_ok` | gauge | `check`(memory-limits/environment-keys/app-identity/grafana/drift-fresh) | per check of the deploy's own read-back (HD-299): `1` passed, `2` **did not apply on this box** (nothing here declares what it reads), `0` failed or has not run yet. `1` and `2` are different on purpose — with one value for both, a crash-looping Grafana nobody asked about read exactly like a Grafana that was checked and found healthy. Written pessimistically — every series `0` — at the deploy's first mutation and rewritten after step 10, so a deploy killed in between leaves a `0` rather than the previous run's `1`. A run that **skipped** a check the previous run read as failing republishes that `0` instead of `2`: a check that did not read the box may lower confidence, never raise it. **Read by `DeployVerifyFailed`, unaggregated** |
| `hamstrack_deploy_verify_checks_ran` | gauge | — | how many of those checks actually READ the running box on the last run. A check skipped because nothing on this box declares what it reads publishes `check_ok` `2` and is not counted here, so the per-check gauge alone cannot tell "everything passed" from "almost nothing was looked at" — this is the denominator that can, and it is also what tells a pessimistic file (`0`) from a finished verify. **No alert and no panel by decision** (see below) |
| `hamstrack_deploy_verify_ok` | gauge | — | the roll-up: `1` when the last verify found nothing wrong, and never `1` while any `check_ok` beside it is `0`. **No alert and no panel by decision:** alerting on it would double every page `DeployVerifyFailed` already sends, with a label that names nothing. It exists for Grafana **Explore** and for the post-deploy read-back in `docs/release-checklist.md` — a human reader, written down here because a metric nobody reads is a metric nobody notices breaking |
| `hamstrack_deploy_verify_timestamp_seconds` | gauge | — | when that gauge was last written. Same decision: Explore and the read-back, no rule. Deliberately **not** a staleness alert — "no deploy lately" is not a finding |
| `hamstrack_deploy_verify_info` | gauge | `sha` | always `1`; the label is the commit the last verified (or verifying) deploy applied. Same decision: Explore and the read-back |
...
| DeployVerifyFailed | `hamstrack_deploy_verify_check_ok < 1` (per `check`: memory-limits, environment-keys, app-identity, grafana, drift-fresh) — written pessimistically at a deploy's first mutation, rewritten by its verify step (HD-299) | 5m | critical |
...
| ScheduledJobStale | `max by (task) ((time() - hamstrack_scheduled_job_last_run_timestamp_seconds) / (hamstrack_scheduled_job_period_seconds + 900 + 2 * clamp_max(hamstrack_scheduled_job_period_seconds, 1800))) > 1` — the bound is `min(3·period + 900, period + 4500)` s per task, spelled with a scalar `clamp_max` because PromQL has no vector `min`: three periods plus a deploy allowance up to a 30-minute period (30 s → 16 m 30 s, 5 min → 30 min, 10 min → 45 min, unchanged), one period plus 75 min above it (hourly → 2 h 15 min, daily → 25 h 15 min — three daily periods is 72 h, longer than any gap between deploys, and a deploy re-seeds `last_run`, so the uncapped bound never fired for a daily task); an unreadable trigger (`period = 0`) → 15 min. **NoData → Alerting** (absence is the finding) | 15m | warning |
| ScheduledJobFailing | `sum by (task) (increase(hamstrack_scheduled_job_failures_total[1h])) > 0` | 0m | warning |
| MailDeadLetterSkipped | `sum(increase(hamstrack_mail_dead_letter_skipped_total{reason!="best_effort"}[1h])) > 0` | 0m | warning |

**Every ops mechanism has a witness, and the witnesses are enumerated from the mechanisms**
(HD-298, `OpsWitnessContractTest` + `ScheduledJobHeartbeatTest`): every `ops/**/*.timer` has a
freshness rule whose `threshold + for` exceeds its schedule (and stays within four periods);
every alert rule resolves to a contact point that exists, and every reached contact point has a
**dated delivery drill** ≤ 60 days old in `docs/ops-prod-hardening.md` § 4.1 — the address behind
`OBS_ALERT_EMAIL_TO` lives outside the repository, so the only thing that proves the inbox is real
is a message that arrived; every registered scheduled task publishes the heartbeat above before it
first runs; every label constant `ProductMetrics` declares is emitted somewhere, every limiter
refusal counts `hamstrack_ratelimit_hit_total`, and every mail-drop branch counts its reason. Each
enumeration carries a floor, and the drill assertion is **red until the owner writes the row** —
