...
| `hamstrack_auth_signup_refused_total` | counter | `reason`(signup_closed/terms_not_accepted/published_password/unencodable_password) | `POST /api/auth/register` refused before it looked at the address (HD-261): the door is closed (`app.registration.public-signup-enabled=false`, the DC default — it answered 403 for months with no witness at all), the request did not accept the terms, the password is one this repository publishes (`DataSeeder.PUBLISHED_PASSWORDS` — the reason with security value: somebody is trying a credential they read here), or the password is over 72 UTF-8 bytes and BCrypt could not hash it. The two password refusals are shared with the reset door and counted by register only — a reset is not a signup. **A knock on a closed door is information, not an incident — there is deliberately no alert rule**; this row and the counter are its witness. Status codes are unchanged: a counter is not an oracle |
