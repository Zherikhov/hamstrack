...
shell "docker exec $APP_CONTAINER env | grep -E '^(DB_POOL_MAX_SIZE|DB_STATEMENT_TIMEOUT_MS|DB_LOCK_TIMEOUT_MS|DB_CONNECTION_TIMEOUT_MS|REPORTS_MAX_ROWS|REPORTS_MAX_WINDOW_DAYS|REPORTS_REQUESTS_PER_MINUTE|SEARCH_REQUESTS_PER_MINUTE|PLANNING_REQUESTS_PER_MINUTE|EXPENSIVE_READ_LIMIT_ENABLED|EXPENSIVE_READ_MAX_IN_FLIGHT|EXPENSIVE_READ_MAX_IN_FLIGHT_PER_PRINCIPAL|EXPENSIVE_READ_ACQUIRE_WAIT_MS|BOARD_MAX_ISSUES|AGILE_SECTION_MAX_ISSUES|AGILE_MAX_OPEN_SPRINTS|MAX_LABELS_PER_WORKSPACE|RATE_LIMIT_ENABLED|RATE_LIMIT_AUTH_IP_PER_MINUTE|RATE_LIMIT_TRUST_FORWARDED_FOR|SPRING_PROFILES_ACTIVE|APP_STORAGE_TYPE)=' | sort"
