          ...
          {users.map(u => {
            ...
            return (
              <tr key={u.id} className="border-b" style={{ borderColor: 'var(--color-border)' }}>
                ...
                <td className="px-3 py-2.5">
                  <Badge label={ROLE_LABEL[u.systemRole]}
                         color={u.systemRole === 'ADMIN' ? 'var(--color-brand)' : undefined} />
                </td>
                <td className="px-3 py-2.5">
                  <span className="inline-flex items-center gap-1.5">
                    <Badge label={u.status.charAt(0) + u.status.slice(1).toLowerCase()} color={STATUS_COLOR[u.status]} />
                    {!u.hasPassword && (
                ...
                <td className="px-3 py-2.5 text-right whitespace-nowrap">
                  ...
                  {!isSelf && (
                    <>
                      ...
                      <Button variant="ghost" size="sm"
                              style={u.status === 'DISABLED' ? undefined : { color: 'var(--color-error)' }}
