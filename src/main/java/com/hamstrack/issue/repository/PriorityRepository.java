...
public interface PriorityRepository extends JpaRepository<Priority, UUID> {
    ...
    /** A name already visible to a scope — forces reuse of an inherited priority. */
    @Query("select case when count(p) > 0 then true else false end from Priority p "
            + "where ((p.scopeWorkspaceId is null and p.scopeProjectId is null) "
            + "or p.scopeWorkspaceId = :ws or p.scopeProjectId = :proj) and p.name = :name")
