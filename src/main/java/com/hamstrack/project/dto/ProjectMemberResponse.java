 ...
 *     and the schema permits a workspace-owned role keyed {@code MANAGER} beside the
 ...
 *     {@code ProjectRole} enum serialised as. Kept beside {@code roleId} rather than
 *     replaced by it: S5's screens are live and read it, and each slice has to be
 *     independently deployable — the same rule {@code roleId}-beside-{@code role} follows
 *     on the request side (S4). On the write paths both fields report what was
 *     <em>stored</em>, which is what makes them agree after the legacy
 *     {@code VIEWER → Contributor} translation.
 *
...
public record ProjectMemberResponse(
     ...
     */
    public static ProjectMemberResponse of(ProjectMember m, RoleView role) {
        var u = m.getUser();
