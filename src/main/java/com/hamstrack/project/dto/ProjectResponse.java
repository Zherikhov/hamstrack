...
public record ProjectResponse(
        ...
        // The caller's EXPLICIT project role, or VIEWER when they have no project_members
        // row. Byte-identical to pre-HD-123 on purpose: S1 is a dark slice and must change
        // nothing a client can observe, so this deliberately does NOT report the effective
        // role a member inherits from the project-access default (§5.2) — `myPermissions`
        // below carries that truth instead. The two therefore disagree for a member with no
        // explicit row (myRole VIEWER, but a full Contributor permission set), which is
        // exactly today's reality: VIEWER has always been the everybody-fallback that grants
        // everything (§2.2). S5 replaces this field's meaning together with the SPA gates
        // that read it. Display only from S5: `myRole === 'MANAGER'` cannot express a custom
        // role, so a component that reads it for a decision is wrong by construction (§5.3).
        ProjectRole myRole,
    ...
    public static ProjectResponse of(Project p, ProjectRole explicitRole, PermissionSet permissions) {
